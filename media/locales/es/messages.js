/**
 * @license
 * Copyright 2025 Singular Blockly Contributors
 * SPDX-License-Identifier: Apache-2.0
 *
 * This file contains translations derived from Blockly project
 * Copyright 2012 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */

// Define translations for Spanish
window.languageManager.loadMessages('es', {
	BLOCKLY_ARIA_CONFIGURATION_ICON: 'Configuración',
	BLOCKLY_ARIA_LOCKED_ICON: 'Bloqueado',
	// UI Elements
	BLOCKS_TAB: 'Bloques',
	CODE_TAB: 'Código',
	BOARD_SELECT_LABEL: 'Seleccionar placa:',
	LANGUAGE_SELECT_TOOLTIP: 'Seleccionar idioma',
	LANGUAGE_AUTO: 'Automático (seguir VS Code)',
	CYBERBRICK_UPLOAD_SETTINGS_BUTTON_TITLE: 'CyberBrick upload settings',
	CYBERBRICK_UPLOAD_SETTINGS_TITLE: 'CyberBrick Upload Settings',
	CYBERBRICK_UPLOAD_READINESS_TITLE: 'OTA Readiness',
	CYBERBRICK_UPLOAD_READINESS_UNKNOWN: 'Open settings to check OTA readiness.',
	CYBERBRICK_UPLOAD_READINESS_READY: 'Ready for OTA upload.',
	CYBERBRICK_UPLOAD_READINESS_NEEDS_SETUP: 'OTA needs setup before upload.',
	CYBERBRICK_UPLOAD_SETTINGS_SAVE: 'Save',
	CYBERBRICK_UPLOAD_SETTINGS_CANCEL: 'Cerrar',
	CYBERBRICK_UPLOAD_SETTINGS_SAVED: 'CyberBrick upload settings saved.',
	CYBERBRICK_UPLOAD_SETTINGS_LOAD_FAILED: 'Could not load CyberBrick upload settings.',
	CYBERBRICK_UPLOAD_SETTINGS_SAVE_FAILED: 'Could not save CyberBrick upload settings.',
	CYBERBRICK_PAIRED_DEVICES_TITLE: 'Mis CyberBricks',
	CYBERBRICK_PROVISIONING_TITLE: 'Agregar carga inalámbrica para CyberBrick',
	CYBERBRICK_NAME_ERROR_EMPTY: 'Escribe un nombre.',
	CYBERBRICK_NAME_ERROR_STARTS_WITH_NUMBER: 'El nombre no puede comenzar con un número.',
	CYBERBRICK_NAME_ERROR_CONTAINS_WHITESPACE: 'Quita los espacios del nombre.',
	CYBERBRICK_NAME_ERROR_CONTAINS_HYPHEN: 'Usa un guión bajo (_) en lugar de un guión (-).',
	CYBERBRICK_NAME_ERROR_INVALID_CHARACTER: 'Usa solo letras, números, guiones bajos o caracteres chinos.',
	CYBERBRICK_NAME_ERROR_PYTHON_KEYWORD: 'Es una palabra reservada de MicroPython. Elige otro nombre.',
	CYBERBRICK_NAME_ERROR_DUPLICATE_FUNCTION: 'Otra función ya usa este nombre.',
	CYBERBRICK_NAME_ERROR_DUPLICATE_PARAMETER: 'Esta función ya tiene un parámetro con este nombre.',
	CYBERBRICK_NAME_WARNING_RUNTIME: 'Este nombre puede ocultar una función del dispositivo MicroPython.',
	CYBERBRICK_NAME_WARNING_BUILTIN: 'Este nombre puede ocultar una función integrada de Python.',
	CYBERBRICK_NAME_UPLOAD_BLOCKED: 'Corrige los nombres de CyberBrick marcados antes de cargar.',
	CYBERBRICK_PROVISION_RUNNING: 'Configurando la carga inalámbrica',
	CYBERBRICK_PROVISION_CURRENT_STAGE: 'Paso actual',
	CYBERBRICK_PROVISION_KEEP_CONNECTED: 'No vuelvas a pulsar ni desconectes el cable USB.',
	CYBERBRICK_PROVISION_RETRY: 'Revisa este paso e inténtalo de nuevo.',
	CYBERBRICK_PROVISION_IN_PROGRESS: 'La configuración inalámbrica ya está en curso.',
	CYBERBRICK_USB_PORT_LABEL: 'USB Port',
	CYBERBRICK_USB_PORT_REFRESH: 'Refresh',
	CYBERBRICK_FRIENDLY_NAME_LABEL: 'Ponle un nombre a tu CyberBrick',
	CYBERBRICK_WIFI_SSID_LABEL: 'Nombre de Wi-Fi',
	CYBERBRICK_WIFI_SSID_EMPTY_PLACEHOLDER: 'No se encontraron redes Wi-Fi',
	CYBERBRICK_WIFI_SCAN_BUTTON: 'Rescan',
	CYBERBRICK_WIFI_PASSWORD_LABEL: 'Wi-Fi Password',
	CYBERBRICK_WIFI_PASSWORD_SHOW: 'Mostrar contraseña Wi-Fi',
	CYBERBRICK_WIFI_PASSWORD_HIDE: 'Ocultar contraseña Wi-Fi',
	CYBERBRICK_PROVISION_BUTTON: 'Configurar carga inalámbrica',
	CYBERBRICK_OTA_CLEANUP_TITLE: 'Configuración avanzada',
	CYBERBRICK_OTA_CLEANUP_DESCRIPTION: 'Remove Singular Blockly OTA files from the USB-connected CyberBrick and return this project to USB-only upload.',
	CYBERBRICK_OTA_CLEANUP_BUTTON: 'Eliminar carga inalámbrica',
	CYBERBRICK_OTA_CLEANUP_DEVICE_REQUIRED: 'Choose a CyberBrick USB port before removing OTA from the device.',
	CYBERBRICK_OTA_CLEANUP_CONFIRM:
		'This will remove Singular Blockly OTA files from the USB-connected CyberBrick, strip the OTA startup block from /app/rc_main.py, delete matching local pairing secrets if found, and switch this project back to USB. It will not touch /boot.py or factory files. Continue?',
	CYBERBRICK_OTA_CLEANUP_RUNNING: 'Removing OTA files over USB...',
	CYBERBRICK_OTA_CLEANUP_SUCCEEDED: 'OTA files removed. Matching local pairing was cleared if found. Upload mode is now USB.',
	CYBERBRICK_OTA_CLEANUP_FAILED: 'Could not remove OTA files from CyberBrick.',
	CYBERBRICK_USB_PORT_REQUIRED: 'Choose a CyberBrick USB port first.',
	CYBERBRICK_PROVISION_REQUIRED_FIELDS: 'Choose a USB port and Wi-Fi SSID first.',
	CYBERBRICK_USB_PORT_PLACEHOLDER: 'Select USB port',
	CYBERBRICK_PAIRED_DEVICES_EMPTY: 'Aún no hay CyberBrick agregados. Expande «Agregar carga inalámbrica para CyberBrick» más abajo.',
	CYBERBRICK_DEVICE_IP_UNKNOWN: 'IP not known',
	CYBERBRICK_SET_PRIMARY_BUTTON: 'Seleccionar',
	CYBERBRICK_DELETE_DEVICE_BUTTON: 'Eliminar',
	CYBERBRICK_WIFI_PASSWORD_STORED: 'A Wi-Fi password is stored securely for the selected device.',
	CYBERBRICK_WIFI_PASSWORD_NOT_STORED: 'La contraseña Wi-Fi se guardará de forma segura tras la configuración.',
	CYBERBRICK_DELETE_DEVICE_CONFIRM: '¿Eliminar este CyberBrick?',
	CYBERBRICK_USB_PORTS_FAILED: 'Could not list CyberBrick USB ports.',
	CYBERBRICK_WIFI_SCAN_RUNNING: 'Scanning Wi-Fi from CyberBrick...',
	CYBERBRICK_PROVISION_STEP_DETECT_USB: 'Puerto USB de CyberBrick seleccionado.',
	CYBERBRICK_PROVISION_STEP_READ_DEVICE_ID_FOUND: 'Identidad del dispositivo encontrada.',
	CYBERBRICK_PROVISION_STEP_READ_DEVICE_ID_CREATING: 'Creando identidad del dispositivo.',
	CYBERBRICK_PROVISION_STEP_READ_DEVICE_ID_CREATED: 'Identidad del dispositivo creada.',
	CYBERBRICK_PROVISION_STEP_INSTALL_AGENT: 'Agente OTA instalado.',
	CYBERBRICK_PROVISION_STEP_CONFIGURE_WIFI: 'Wi-Fi configurado.',
	CYBERBRICK_PROVISION_STEP_CONFIGURE_WIFI_NO_IP: 'Configuración Wi-Fi guardada; aún no se informó la IP.',
	CYBERBRICK_PROVISION_STEP_STORE_SECRETS: 'Secretos guardados de forma segura.',
	CYBERBRICK_PROVISION_STEP_VERIFY_AGENT: 'Comprobando agente OTA.',
	CYBERBRICK_PROVISION_STEP_DETECT_USB_FAILED: 'No se pudo usar la conexión USB de CyberBrick.',
	CYBERBRICK_PROVISION_STEP_READ_DEVICE_ID_FAILED: 'No se pudo leer ni crear la identidad del dispositivo.',
	CYBERBRICK_PROVISION_STEP_INSTALL_AGENT_FAILED: 'No se pudo instalar el agente OTA.',
	CYBERBRICK_PROVISION_STEP_CONFIGURE_WIFI_FAILED: 'No se pudo completar la configuración Wi-Fi.',
	CYBERBRICK_PROVISION_STEP_VERIFY_AGENT_FAILED: 'No se pudo verificar el agente OTA.',
	CYBERBRICK_PROVISION_STEP_STORE_SECRETS_FAILED: 'No se pudo guardar la información de emparejamiento y seguridad.',
	CYBERBRICK_PROVISION_STEP_UNKNOWN: 'Paso de configuración OTA actualizado.',
	CYBERBRICK_PROVISION_SUCCEEDED: 'Configuración OTA de CyberBrick completada. El modo de carga sigue en USB.',
	CYBERBRICK_OTA_RC_CHANNEL_NOTE: 'Nota de canal RC: Ambos con Wi-Fi → canal Wi-Fi, RC y OTA funcionan. Sin Wi-Fi → canal configurado, RC funciona. Uno con Wi-Fi, otro sin → RC puede fallar al emparejar.',
	CYBERBRICK_PROVISION_FAILED: 'Error al configurar OTA de CyberBrick.',
	CYBERBRICK_OTA_UPLOAD_SUCCEEDED: 'CyberBrick OTA upload complete.',
	CYBERBRICK_OTA_UPLOAD_FAILED: 'CyberBrick OTA upload failed. USB fallback is manual only.',
	CYBERBRICK_DEVICE_DELETED: 'Paired CyberBrick deleted.',
	CYBERBRICK_DEVICE_DELETE_FAILED: 'Could not delete paired CyberBrick.',
	CYBERBRICK_ERROR_AGENT_UNREACHABLE: 'No se pudo contactar el agente OTA de CyberBrick.',
	CYBERBRICK_ERROR_OFFLINE: 'CyberBrick parece estar desconectado.',
	CYBERBRICK_ERROR_AGENT_HEALTH: 'El agente OTA de CyberBrick no está listo.',
	CYBERBRICK_ERROR_MISSING_PRIMARY: 'No hay CyberBrick seleccionado para OTA.',
	CYBERBRICK_ERROR_NOT_PAIRED: 'CyberBrick no está configurado para OTA.',
	CYBERBRICK_ERROR_MISSING_ADDRESS: 'La dirección IP de CyberBrick es desconocida.',
	CYBERBRICK_ERROR_MISSING_TOKEN: 'CyberBrick no tiene token OTA.',
	CYBERBRICK_ERROR_TOKEN_REJECTED: 'CyberBrick rechazó el token OTA.',
	CYBERBRICK_ERROR_IDENTITY_MISMATCH: 'CyberBrick rechazó la solicitud de carga.',
	CYBERBRICK_ERROR_WRITE_FAILED: 'La escritura OTA de CyberBrick falló.',
	CYBERBRICK_ERROR_NETWORK: 'Error de red al conectar con CyberBrick.',
	CYBERBRICK_ERROR_TIMEOUT: 'La carga OTA de CyberBrick agotó el tiempo.',
	CYBERBRICK_ERROR_OTA_FAILED: 'La carga OTA de CyberBrick falló.',
	CYBERBRICK_ERROR_RC_MAIN_PATCH_FAILED: 'El parche OTA de CyberBrick falló.',
	OTA_AGENT_UPGRADING: 'Actualizando agente OTA {0} → {1}...',
	OTA_AGENT_UPGRADED: 'Agente OTA actualizado a {0}. Reanudando carga...',
	OTA_AGENT_UPGRADE_NEEDED: 'El agente OTA {0} está desactualizado. Vuelva a emparejar el CyberBrick para habilitar la actualización automática.',

	// Experimental Blocks Notice
	EXPERIMENTAL_BLOCKS_TITLE: 'Bloques experimentales detectados',
	EXPERIMENTAL_BLOCKS_DESC:
		'Su espacio de trabajo contiene bloques experimentales (resaltados con bordes amarillos discontinuos). Estas funciones pueden cambiar o eliminarse en futuras actualizaciones, úselas con precaución.',

	// Preview Mode UI
	PREVIEW_BADGE: 'Vista previa',
	THEME_TOGGLE: 'Cambiar tema',
	TOOLBAR_ACTIONS_EXPAND: 'Mostrar más acciones',
	TOOLBAR_ACTIONS_COLLAPSE: 'Mostrar menos acciones',
	PREVIEW_WINDOW_TITLE: 'Vista previa Blockly - {0}',
	PREVIEW_WINDOW_TITLE_WITH_NAME: 'Vista previa - {0}',

	// Backup Modal UI
	BACKUP_MANAGER_TITLE: 'Gestor de copias de seguridad',
	BACKUP_CREATE_NEW: 'Crear nueva copia',
	BACKUP_NAME_LABEL: 'Nombre:',
	BACKUP_NAME_PLACEHOLDER: 'Introduzca nombre de la copia',
	BACKUP_CONFIRM: 'Confirmar',
	BACKUP_CANCEL: 'Cancelar',
	BACKUP_LIST_TITLE: 'Lista de copias',
	BACKUP_LIST_EMPTY: 'No hay copias disponibles',
	BACKUP_BUTTON_TITLE: 'Gestor de copias',
	REFRESH_BUTTON_TITLE: 'Actualizar código',
	BACKUP_PREVIEW_BTN: 'Previsualizar',
	BACKUP_RESTORE_BTN: 'Restaurar',
	BACKUP_DELETE_BTN: 'Eliminar',
	AUTO_BACKUP_TITLE: 'Configuración de copia automática',
	AUTO_BACKUP_INTERVAL_LABEL: 'Intervalo de copia:',
	AUTO_BACKUP_MINUTES: 'minutos',
	AUTO_BACKUP_SAVE: 'Guardar configuración',
	AUTO_BACKUP_SAVED: 'Configuración de copia automática guardada',
	AUTO_BACKUP_PREFIX: 'auto_',
	MANUAL_BACKUP_TITLE: 'Copia manual',

	// Board Names
	BOARD_NONE: 'Ninguna',
	BOARD_UNO: 'Arduino Uno',
	BOARD_NANO: 'Arduino Nano',
	BOARD_MEGA: 'Arduino Mega',
	BOARD_ESP32: 'ESP32',
	BOARD_SUPERMINI: 'Super Mini',
	// Block Search UI
	FUNCTION_SEARCH_TITLE: 'Buscar Bloques',
	FUNCTION_SEARCH_BUTTON_TITLE: 'Buscar Bloques',
	FUNCTION_SEARCH_PLACEHOLDER: 'Introduzca nombre o parámetros del bloque...',
	FUNCTION_SEARCH_BTN: 'Buscar',
	FUNCTION_SEARCH_PREV: 'Anterior',
	FUNCTION_SEARCH_NEXT: 'Siguiente',
	FUNCTION_SEARCH_EMPTY: 'Aún no se ha buscado',
	FUNCTION_SEARCH_NO_RESULTS: 'No se encontraron bloques coincidentes',
	FUNCTION_RESULT_PREFIX: 'Bloque: ',
	FUNCTION_KEYBOARD_SHORTCUT_TIP: '(Atajo: Ctrl+F)',
	FUNCTION_SEARCH_HISTORY_TITLE: 'Historial de búsqueda',

	// Block Categories
	CATEGORY_LOGIC: 'Lógica',
	CATEGORY_LOOPS: 'Bucles',
	CATEGORY_MATH: 'Matemáticas',
	CATEGORY_TEXT: 'Texto',
	CATEGORY_LISTS: 'Listas',
	CATEGORY_VARIABLES: 'Variables',
	CATEGORY_FUNCTIONS: 'Funciones',
	CATEGORY_ARDUINO: 'Arduino',
	CATEGORY_SENSORS: 'Sensores',
	CATEGORY_MOTORS: 'Motores',
	VISION_SENSORS_CATEGORY: 'Sensores de Visión',
	// Servo Block Labels
	SERVO_SETUP: 'Configurar Motor Servo',
	SERVO_PIN: 'Pin',
	SERVO_SETUP_TOOLTIP: 'Declarar variable de motor servo y establecer pin',
	SERVO_MOVE: 'Girar Motor Servo',
	SERVO_ANGLE: 'Ángulo',
	SERVO_MOVE_TOOLTIP: 'Girar motor servo a un ángulo específico',
	SERVO_STOP: 'Detener Motor Servo',
	SERVO_STOP_TOOLTIP: 'Detener señal de salida del motor servo',

	// Encoder Motor Control
	ENCODER_SETUP: 'Configurar Motor Encoder',
	ENCODER_NAME: 'Nombre',
	ENCODER_PIN_A: 'Pin A',
	ENCODER_PIN_B: 'Pin B',
	ENCODER_USE_INTERRUPT: 'Usar Interrupción',
	ENCODER_SETUP_TOOLTIP: 'Configurar motor encoder con configuraciones de nombre y pines',
	ENCODER_READ: 'Leer Encoder',
	ENCODER_READ_TOOLTIP: 'Obtener posición actual del encoder',
	ENCODER_RESET: 'Resetear Encoder',
	ENCODER_RESET_TOOLTIP: 'Resetear posición del encoder a cero',
	ENCODER_PID_SETUP: 'Configurar Control PID',
	ENCODER_PID_MOTOR: 'Motor',
	ENCODER_PID_KP: 'Kp',
	ENCODER_PID_KI: 'Ki',
	ENCODER_PID_KD: 'Kd',
	ENCODER_PID_MODE: 'Modo',
	ENCODER_PID_MODE_POSITION: 'Posición',
	ENCODER_PID_MODE_SPEED: 'Velocidad',
	ENCODER_PID_SETUP_TOOLTIP: 'Configurar control PID para control preciso del motor. Seleccione modo de posición o velocidad.',
	ENCODER_PID_COMPUTE: 'Calcular PID',
	ENCODER_PID_TARGET: 'Objetivo',
	ENCODER_PID_COMPUTE_TOOLTIP: 'Calcular salida de control PID basado en posición objetivo',
	ENCODER_PID_RESET: 'Reiniciar PID',
	ENCODER_PID_RESET_TOOLTIP: 'Reiniciar el estado del controlador PID (borrar acumulación integral, reiniciar contador)',

	// Arduino Block Labels
	ARDUINO_SETUP: 'Configuración',
	ARDUINO_LOOP: 'Bucle',
	ARDUINO_DIGITAL_WRITE: 'Escribir Digital',
	ARDUINO_DIGITAL_READ: 'Leer Digital',
	ARDUINO_ANALOG_WRITE: 'Escribir Analógico',
	ARDUINO_ANALOG_READ: 'Leer Analógico',
	ARDUINO_PIN: 'Pin',
	ARDUINO_VALUE: 'Valor',
	ARDUINO_DELAY: 'Retardo',
	ARDUINO_DELAY_MS: 'milisegundos',
	ARDUINO_PULLUP: 'Activar Resistencia Interna',
	ARDUINO_MODE: 'Modo',
	ARDUINO_MODE_INPUT: 'ENTRADA',
	ARDUINO_MODE_OUTPUT: 'SALIDA',

	// Sensor Block Labels
	ULTRASONIC_SENSOR: 'Sensor Ultrasónico',
	ULTRASONIC_TRIG_PIN: 'Pin Trig',
	ULTRASONIC_ECHO_PIN: 'Pin Echo',
	ULTRASONIC_USE_INTERRUPT: 'Usar Interrupción Hardware',
	ULTRASONIC_READ: 'Leer Distancia Ultrasónica (cm)',
	ULTRASONIC_TOOLTIP_SETUP: 'Configura el sensor ultrasónico con pines Trig y Echo. Interrupción hardware opcional para mayor precisión.',
	ULTRASONIC_TOOLTIP_READ: 'Lee la distancia medida por el sensor ultrasónico en centímetros.',
	ULTRASONIC_WARNING: 'El pin Echo {0} seleccionado no soporta interrupciones hardware. Por favor, elija uno de estos pines: {1}',

	// Threshold Function Block Labels
	THRESHOLD_PIN: 'en',
	THRESHOLD_VALUE: 'si >',
	THRESHOLD_HIGH_VALUE: '=',
	THRESHOLD_LOW_VALUE: 'sino',
	THRESHOLD_TOOLTIP_SETUP:
		'Configura una función de umbral. Cuando la entrada analógica excede el umbral, devuelve la primera salida, de lo contrario devuelve la segunda.',
	THRESHOLD_TOOLTIP_READ: 'Obtener el valor de la función de umbral',

	// Duration block
	DURATION_REPEAT: 'Repetir por',
	DURATION_TIME: 'tiempo',
	DURATION_MS: 'milisegundos',
	DURATION_DO: 'hacer',

	// Print block
	TEXT_PRINT_SHOW: 'imprimir',
	TEXT_PRINT_NEWLINE: 'nueva línea',

	// Pin Mode block
	PIN_MODE_SET: 'establecer',

	// Function Block Labels
	FUNCTION_CREATE: 'Crear Función',
	FUNCTION_NAME: 'Nombre',
	FUNCTION_PARAMS: 'Parámetros',
	FUNCTION_RETURN: 'Retorno',
	FUNCTION_CALL: 'Llamar',
	FUNCTION_LOCK_BLOCK: 'Bloquear bloque',
	FUNCTION_UNLOCK_BLOCK: 'Desbloquear bloque',

	// Logic Block Labels
	LOGIC_IF: 'si',
	LOGIC_ELSE: 'sino',
	LOGIC_THEN: 'entonces',
	LOGIC_AND: 'y',
	LOGIC_OR: 'o',
	LOGIC_NOT: 'no',
	LOGIC_TRUE: 'verdadero',
	LOGIC_FALSE: 'falso',

	// Loop Block Labels
	LOOP_REPEAT: 'repetir',
	LOOP_WHILE: 'mientras',
	LOOP_UNTIL: 'hasta que',
	LOOP_FOR: 'para',
	LOOP_FOREACH: 'para cada',
	LOOP_BREAK: 'romper',
	LOOP_CONTINUE: 'continuar',

	// Math Block Labels
	MATH_NUMBER: 'número',
	MATH_ARITHMETIC: 'aritmética',
	MATH_OPERATIONS: 'operaciones',
	MATH_ADD: 'sumar',
	MATH_SUBTRACT: 'restar',
	MATH_MULTIPLY: 'multiplicar',
	MATH_DIVIDE: 'dividir',
	MATH_POWER: 'potencia',

	// Math Map Block
	MATH_MAP_VALUE: 'mapear',
	MATH_MAP_TOOLTIP:
		'Remapea un número de un rango a otro. Por ejemplo, map(valor, 0, 1023, 0, 255) escalará una entrada analógica a una salida PWM de 8 bits.',

	// VS Code UI Messages
	VSCODE_OPEN_FOLDER: 'Abrir Carpeta',
	VSCODE_PLEASE_OPEN_PROJECT: '¡Por favor, abra primero una carpeta de proyecto!',
	VSCODE_FAILED_SAVE_FILE: 'Error al guardar el archivo: {0}',
	VSCODE_FAILED_UPDATE_INI: 'Error al actualizar platformio.ini: {0}',
	VSCODE_UNABLE_SAVE_WORKSPACE: 'No se pudo guardar el estado del espacio de trabajo: {0}',
	VSCODE_FAILED_START: 'Error al iniciar Singular Blockly: {0}',
	VSCODE_CONFIRM_DELETE_VARIABLE: '¿Está seguro de que desea eliminar la variable "{0}"?',
	VSCODE_BOARD_UPDATED: 'Configuración de la placa actualizada a: {0}',
	VSCODE_RELOAD_REQUIRED: '，Por favor, recargue la ventana para completar la configuración',
	VSCODE_ENTER_VARIABLE_NAME: 'Introduzca el nombre de la nueva variable',
	VSCODE_ENTER_NEW_VARIABLE_NAME: 'Introduzca el nuevo nombre de la variable (actual: {0})',
	VSCODE_VARIABLE_NAME_EMPTY: 'El nombre de la variable no puede estar vacío',
	VSCODE_VARIABLE_NAME_INVALID:
		'El nombre de la variable solo puede contener letras, números y guiones bajos, y no puede comenzar con un número',
	VSCODE_VARIABLE_NAME_INVALID_MICROPYTHON:
		'El nombre de la variable solo puede contener caracteres chinos, letras, números y guiones bajos, y no puede comenzar con un número',
	VSCODE_RELOAD: 'Recargar',
	VSCODE_OK: 'Aceptar',
	VSCODE_CANCEL: 'Cancelar',
	VSCODE_OPEN_BLOCKLY_EDITOR: 'Abrir Editor Blockly',
	WORKSPACE_CANDIDATE_INVALID_WARNING: 'Se aisló un cambio externo no válido del espacio de trabajo y se restauró la última versión válida. Detalles: blockly/main.invalid.json',
	WORKSPACE_CANDIDATE_QUARANTINED_WARNING: 'Se aisló un cambio externo no válido del espacio de trabajo, pero no había una última versión válida disponible para restaurar. Detalles: blockly/main.invalid.json',
	WORKSPACE_CANDIDATE_SHOW_DETAILS: 'Mostrar detalles de salida',
	// PlatformIO Diagnostic Panel
	PLATFORMIO_DIAGNOSTIC_PANEL_TITLE: 'PlatformIO Diagnostic',
	MANAGED_RUNTIME_PROGRESS_INITIALIZE_TITLE: 'Preparando Singular Core',
	MANAGED_RUNTIME_PROGRESS_REPAIR_TITLE: 'Reparando Singular Core',
	MANAGED_RUNTIME_PROGRESS_STAGE_PERCENT: '{0} ({1} %)',
	MANAGED_RUNTIME_PROGRESS_WAITING_LOCK: 'Esperando a que otra ventana termine de preparar Singular Core',
	MANAGED_RUNTIME_PROGRESS_DOWNLOADING_PYTHON: 'Descargando el entorno de ejecución administrado de Python',
	MANAGED_RUNTIME_PROGRESS_EXTRACTING_PYTHON: 'Extrayendo el entorno de ejecución administrado de Python',
	MANAGED_RUNTIME_PROGRESS_INSTALLING_PLATFORMIO: 'Instalando PlatformIO Core',
	MANAGED_RUNTIME_PROGRESS_INSTALLING_MPREMOTE: 'Instalando el asistente de dispositivos CyberBrick',
	MANAGED_RUNTIME_PROGRESS_VERIFYING: 'Verificando las herramientas administradas',
	MANAGED_RUNTIME_PROGRESS_COMMITTING: 'Confirmando Singular Core verificado',
	MANAGED_RUNTIME_PROGRESS_PATH_TOO_LONG: 'Singular Core necesita una carpeta local más corta en este sistema Windows. Provider Core no se modificó.',
	MANAGED_RUNTIME_PROGRESS_FAILED: 'La configuración de Singular Core no se completó. Provider Core no se modificó.',
	MANAGED_RUNTIME_ACTION_OPEN_DIAGNOSTICS: 'Abrir diagnóstico',
	MANAGED_RUNTIME_ACTION_CHOOSE_SHORTER_FOLDER: 'Elegir una carpeta más corta',
	PLATFORMIO_DIAGNOSTIC_PANEL_SUBTITLE:
		'Check whether the extension can resolve PlatformIO CLI, penv, and the CyberBrick helper tools.',
	PLATFORMIO_DIAGNOSTIC_LOADING_TITLE: 'Checking PlatformIO environment...',
	PLATFORMIO_DIAGNOSTIC_LOADING_DESCRIPTION:
		'This usually finishes in a few seconds. If a tool is slow, the panel falls back to a partial result instead of waiting forever.',
	PLATFORMIO_DIAGNOSTIC_ERROR_TITLE: 'Diagnostic error',
	PLATFORMIO_DIAGNOSTIC_SUMMARY_TITLE: 'Summary',
	PLATFORMIO_DIAGNOSTIC_TOOLS_TITLE: 'Resolved tools',
	PLATFORMIO_DIAGNOSTIC_CORE_ENVIRONMENTS_TITLE: 'Entornos Core',
	PLATFORMIO_DIAGNOSTIC_PROVIDER_CORE: 'Core de la extensión PlatformIO',
	PLATFORMIO_DIAGNOSTIC_MANAGED_CORE: 'Core administrado por Singular',
	PLATFORMIO_DIAGNOSTIC_ARDUINO_SELECTION: 'Enrutamiento de Arduino',
	PLATFORMIO_DIAGNOSTIC_PYTHON_SELECTION: 'Enrutamiento de Python',
	PLATFORMIO_DIAGNOSTIC_PACKAGE_STATUS: 'Estado de paquetes',
	PLATFORMIO_DIAGNOSTIC_STORAGE_USAGE: 'Uso del almacenamiento',
	PLATFORMIO_DIAGNOSTIC_FALLBACK_USED: 'Respaldo utilizado',
	PLATFORMIO_DIAGNOSTIC_ACTION_REPAIR_MANAGED_RUNTIME: 'Reparar entorno administrado',
	PLATFORMIO_DIAGNOSTIC_ACTION_CLEANUP_MANAGED_RUNTIME: 'Limpiar archivos administrados',
	PLATFORMIO_DIAGNOSTIC_ACTION_REVEAL_MANAGED_RUNTIME: 'Abrir la carpeta de Singular Core',
	PLATFORMIO_DIAGNOSTIC_MANAGED_REPAIR_STARTED: 'Reparando el entorno administrado. Puede ser necesario descargar archivos.',
	PLATFORMIO_DIAGNOSTIC_MANAGED_REPAIR_SUCCESS: 'Se completó la reparación del entorno administrado.',
	PLATFORMIO_DIAGNOSTIC_MANAGED_REPAIR_FAILED: 'Falló la reparación del entorno administrado. Abre el registro de salida para ver los detalles con los datos confidenciales ocultos.',
	PLATFORMIO_DIAGNOSTIC_MANAGED_CLEANUP_SUCCESS: 'Limpieza completada: se eliminaron {0} descargas, {1} directorios temporales y {2} versiones antiguas.',
	PLATFORMIO_DIAGNOSTIC_MANAGED_CLEANUP_FAILED: 'Falló la limpieza del entorno administrado. Los archivos desconocidos no se modificaron.',
	PLATFORMIO_DIAGNOSTIC_MANAGED_REVEAL_FAILED:
		'No se puede abrir la carpeta de Singular Core. Completa la inicialización o repara el entorno administrado y vuelve a intentarlo.',
	PLATFORMIO_DIAGNOSTIC_SCOPE_TITLE: 'Scope',
	PLATFORMIO_DIAGNOSTIC_WORKSPACE: 'Workspace',
	PLATFORMIO_DIAGNOSTIC_REQUESTED_AT: 'Generated at',
	PLATFORMIO_DIAGNOSTIC_OVERALL_STATUS: 'Overall status',
	PLATFORMIO_DIAGNOSTIC_PATH: 'Path',
	PLATFORMIO_DIAGNOSTIC_PATH_UNRESOLVED: 'Not resolved',
	PLATFORMIO_DIAGNOSTIC_SOURCE: 'Source',
	PLATFORMIO_DIAGNOSTIC_REASON: 'Reason',
	PLATFORMIO_DIAGNOSTIC_NEXT_STEP: 'Next step',
	PLATFORMIO_DIAGNOSTIC_VERSION: 'Version probe',
	PLATFORMIO_DIAGNOSTIC_FROM_DETECTED_PENV: 'From detected penv',
	PLATFORMIO_DIAGNOSTIC_BOOLEAN_YES: 'Yes',
	PLATFORMIO_DIAGNOSTIC_BOOLEAN_NO: 'No',
	PLATFORMIO_DIAGNOSTIC_NO_DATA_TITLE: 'No data yet',
	PLATFORMIO_DIAGNOSTIC_NO_DATA_DESCRIPTION: 'Run the diagnostic once to see resolved paths and tool details.',
	PLATFORMIO_DIAGNOSTIC_ACTION_RETEST: 'Retest',
	PLATFORMIO_DIAGNOSTIC_ACTION_COPY_SUMMARY: 'Copy summary',
	PLATFORMIO_DIAGNOSTIC_COPY_UNAVAILABLE: 'No diagnostic summary is available yet.',
	PLATFORMIO_DIAGNOSTIC_COPY_SUCCESS: 'Diagnostic summary copied to the clipboard.',
	PLATFORMIO_DIAGNOSTIC_COPY_FAILED: 'Unable to copy the diagnostic summary: {0}',
	PLATFORMIO_DIAGNOSTIC_TOP_LEVEL_ERROR: 'Unable to complete PlatformIO diagnostics: {0}',
	PLATFORMIO_DIAGNOSTIC_STATUS_OK: 'OK',
	PLATFORMIO_DIAGNOSTIC_STATUS_WARNING: 'Warning',
	PLATFORMIO_DIAGNOSTIC_STATUS_ERROR: 'Error',
	PLATFORMIO_DIAGNOSTIC_OVERALL_OPERATIONAL: 'Operational',
	PLATFORMIO_DIAGNOSTIC_OVERALL_DEGRADED: 'Degraded',
	PLATFORMIO_DIAGNOSTIC_OVERALL_UNAVAILABLE: 'Unavailable',
	PLATFORMIO_DIAGNOSTIC_SOURCE_DEFAULT_PLATFORMIO_PATH: 'Default PlatformIO path',
	PLATFORMIO_DIAGNOSTIC_SOURCE_PATH_SEARCH: 'PATH search',
	PLATFORMIO_DIAGNOSTIC_SOURCE_RESOLVED_PIO_SIBLING: 'Derived from resolved pio sibling',
	PLATFORMIO_DIAGNOSTIC_SOURCE_DERIVED_FROM_PENV: 'Derived from detected penv',
	PLATFORMIO_DIAGNOSTIC_SOURCE_UNRESOLVED: 'Unresolved',
	PLATFORMIO_DIAGNOSTIC_TOOL_PIO: 'PlatformIO CLI (pio)',
	PLATFORMIO_DIAGNOSTIC_TOOL_PENV_ROOT: 'PlatformIO penv root',
	PLATFORMIO_DIAGNOSTIC_TOOL_PYTHON: 'PlatformIO Python',
	PLATFORMIO_DIAGNOSTIC_TOOL_PIP: 'PlatformIO pip',
	PLATFORMIO_DIAGNOSTIC_TOOL_MPREMOTE: 'mpremote',
	PLATFORMIO_DIAGNOSTIC_SCOPE_NOTICE:
		'This report reflects the VS Code extension runtime on this machine. It may differ from the shell environment you use in an external terminal.',
	PLATFORMIO_DIAGNOSTIC_REASON_EXECUTABLE_MISSING:
		'{0} was not found in the default PlatformIO location or the current PATH visible to the VS Code extension runtime.',
	PLATFORMIO_DIAGNOSTIC_REASON_EXECUTABLE_READY: '{0} was resolved successfully and the version probe completed.',
	PLATFORMIO_DIAGNOSTIC_REASON_VERSION_PROBE_FAILED:
		'{0} was found at {1}, but the version probe did not complete successfully: {2}',
	PLATFORMIO_DIAGNOSTIC_REASON_PENV_DERIVED: '{0} was derived from the resolved PlatformIO CLI location.',
	PLATFORMIO_DIAGNOSTIC_REASON_PENV_MISSING:
		'A penv root was derived from the resolved pio path, but the directory does not exist: {0}',
	PLATFORMIO_DIAGNOSTIC_REASON_PENV_INVALID_LAYOUT:
		'A sibling directory was derived from the resolved pio path, but it does not look like a PlatformIO penv root: {0}',
	PLATFORMIO_DIAGNOSTIC_REASON_PENV_UNRESOLVED:
		'No penv root could be derived because the PlatformIO CLI path is not resolved yet.',
	PLATFORMIO_DIAGNOSTIC_NEXTSTEP_CHECK_PLATFORMIO_INSTALL:
		'Confirm PlatformIO CLI is installed and reachable from the VS Code extension runtime, then run diagnostics again.',
	PLATFORMIO_DIAGNOSTIC_NEXTSTEP_CHECK_TOOL_IN_PENV:
		'Check whether {0} exists inside the detected PlatformIO penv or PATH, then run diagnostics again.',
	PLATFORMIO_DIAGNOSTIC_NEXTSTEP_CHECK_PENV_LAYOUT:
		'Check whether the detected PlatformIO penv directory layout is intact, then run diagnostics again.',
	PLATFORMIO_DIAGNOSTIC_NEXTSTEP_RETRY_AFTER_FIX:
		'Verify the executable can run from this path, fix the environment if needed, and then run diagnostics again.',

	PLATFORMIO_DIAGNOSTIC_SOURCE_OFFICIAL_PLATFORMIO_CUSTOM_PATH: 'PlatformIO customPATH setting',
	PLATFORMIO_DIAGNOSTIC_SOURCE_OFFICIAL_PLATFORMIO_SETTINGS: 'Official PlatformIO settings',
	PLATFORMIO_DIAGNOSTIC_SOURCE_COMMON_DIR: 'Common tool directory',
	PLATFORMIO_DIAGNOSTIC_SOURCE_REPAIR_HISTORY: 'Repair history',
	PLATFORMIO_DIAGNOSTIC_SETTINGS_EVIDENCE: 'PlatformIO settings evidence',
	PLATFORMIO_REPAIR_ACTION_START_AUTO_REPAIR: 'Auto repair',
	PLATFORMIO_REPAIR_ACTION_CONFIRM_AUTO_REPAIR: 'Confirm repair',
	PLATFORMIO_REPAIR_ACTION_CANCEL_AUTO_REPAIR: 'Cancel',
	PLATFORMIO_REPAIR_ACTION_CLEAR_HISTORY: 'Clear history',
	PLATFORMIO_REPAIR_ACTION_COPY_AI_PACKET: 'Copy AI repair summary',
	PLATFORMIO_REPAIR_ACTION_CREATE_ISSUE_DRAFT: 'Enviar comentarios',
	PLATFORMIO_REPAIR_SECTION_TITLE: 'Guided repair',
	PLATFORMIO_REPAIR_EXPORTS_TITLE: 'Support exports',
	PLATFORMIO_REPAIR_RECOMMENDATION_TITLE: 'Recommended repair flow',
	PLATFORMIO_REPAIR_HISTORY_TITLE: 'Repair history',
	PLATFORMIO_REPAIR_PRIMARY_FIX: 'Primary fix',
	PLATFORMIO_REPAIR_FALLBACK_FIX: 'Fallback fix',
	PLATFORMIO_REPAIR_VERIFICATION: 'Verification after repair',
	PLATFORMIO_REPAIR_APPLICABILITY: 'Applicability',
	PLATFORMIO_REPAIR_NOT_APPLICABLE: 'Not applicable',
	PLATFORMIO_REPAIR_MANUAL_ALTERNATIVE: 'Manual alternative',
	PLATFORMIO_REPAIR_MANUAL_STEPS: 'Manual steps still required',
	PLATFORMIO_REPAIR_CONFIRMATION_TITLE: 'Confirm automatic repair',
	PLATFORMIO_REPAIR_PROGRESS_TITLE: 'Repair progress',
	PLATFORMIO_REPAIR_NO_RECOMMENDATION: 'No automatic repair is recommended for the current status.',
	PLATFORMIO_REPAIR_FINGERPRINT_CURRENT: 'Environment matches stored history',
	PLATFORMIO_REPAIR_FINGERPRINT_STALE: 'Some repair history may be stale',
	PLATFORMIO_REPAIR_FINGERPRINT_UNKNOWN: 'Environment fingerprint unavailable',
	PLATFORMIO_REPAIR_AI_PACKET_REDACTION_NOTICE: 'The copied repair packet masks local paths, proxy credentials, and token-like strings.',
	PLATFORMIO_REPAIR_AI_PACKET_STALE_HISTORY_NOTICE: 'Some included repair history may be stale because the environment changed.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_PRIVACY_NOTICE: 'Review the privacy checklist before posting this draft publicly.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_DUPLICATE_NOTICE: 'Search existing issues with the suggested keywords before opening a new issue.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_NO_DRAFT: 'No issue draft recommended',
	PLATFORMIO_REPAIR_REDACTION_NOTICE: 'Exports redact local paths, proxy credentials, and token-like strings by default.',
	PLATFORMIO_REPAIR_RETEST_BLOCKED_ACTIVE_RUN: 'Repair is currently running. Wait for it to finish before retesting.',
	PLATFORMIO_REPAIR_ALREADY_RUNNING: 'A repair run is already in progress.',
	PLATFORMIO_REPAIR_FLOW_NOT_AVAILABLE: 'The selected repair flow is no longer available.',
	PLATFORMIO_REPAIR_EXPORT_NOT_READY: 'This export action is not available until diagnostics finish.',
	PLATFORMIO_REPAIR_FAILED: 'Automatic repair failed: {0}',
	PLATFORMIO_REPAIR_CANCEL_REQUESTED: 'Cancellation requested. The current safe step will finish before stopping.',
	PLATFORMIO_REPAIR_AI_PACKET_UNAVAILABLE: 'No diagnostic data is available for an AI repair packet yet.',
	PLATFORMIO_REPAIR_AI_PACKET_COPY_SUCCESS_STALE: 'AI repair packet copied. Some included repair history may be stale; review before sharing.',
	PLATFORMIO_REPAIR_AI_PACKET_COPY_SUCCESS: 'AI repair packet copied with sensitive details redacted.',
	PLATFORMIO_REPAIR_AI_PACKET_COPY_FAILED: 'Unable to copy the AI repair packet: {0}',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_UNAVAILABLE: 'No diagnostic data is available for an issue draft yet.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_NOT_RECOMMENDED: 'No issue draft is recommended for the current diagnostics.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_COPY_SUCCESS: 'Issue draft copied locally. Review privacy and duplicate-search checklist before posting.',
	PLATFORMIO_REPAIR_FEEDBACK_OPENED: 'Se abrió el formulario con los datos privados de PlatformIO ocultos. Revisa todo lo que se enviará antes de confirmar.',
	PLATFORMIO_REPAIR_FEEDBACK_PREFILL: 'El diagnóstico de PlatformIO no completó el proceso esperado.\nEstado general: {0}.\nComprobaciones afectadas: {1}.\nDescribe qué estabas haciendo y qué resultado esperabas.',
	PLATFORMIO_REPAIR_ISSUE_DRAFT_COPY_FAILED: 'Unable to create the issue draft: {0}',

	// Error Messages
	ERROR_BOARD_NOT_SELECTED: 'Por favor, seleccione una placa primero',
	ERROR_INVALID_PIN: 'Número de pin inválido',
	ERROR_INVALID_VALUE: 'Valor inválido',
	ERROR_MISSING_TRANSLATION: 'Traducción faltante',

	// Blockly core messages
	ADD: 'añadir',
	REMOVE: 'eliminar',
	RENAME: 'renombrar',
	NEW: 'nuevo',
	ADD_COMMENT: 'Añadir Comentario',
	REMOVE_COMMENT: 'Eliminar Comentario',
	DUPLICATE_BLOCK: 'Duplicar',
	HELP: 'Ayuda',
	UNDO: 'Deshacer',
	REDO: 'Rehacer',
	COLLAPSE_BLOCK: 'Colapsar Bloque',
	EXPAND_BLOCK: 'Expandir Bloque',
	DELETE_BLOCK: 'Eliminar Bloque',
	DELETE_X_BLOCKS: 'Eliminar %1 Bloques',
	DELETE_ALL_BLOCKS: '¿Eliminar todos los %1 bloques?',
	CLEAN_UP: 'Ordenar Bloques',
	COLLAPSE_ALL: 'Colapsar Bloques',
	EXPAND_ALL: 'Expandir Bloques',
	DISABLE_BLOCK: 'Deshabilitar Bloque',
	ENABLE_BLOCK: 'Habilitar Bloque',
	INLINE_INPUTS: 'Entradas en Línea',
	EXTERNAL_INPUTS: 'Entradas Externas',

	// Variable & Function messages
	RENAME_VARIABLE: 'Renombrar variable...',
	NEW_VARIABLE: 'Crear variable...',
	DELETE_VARIABLE: 'Eliminar variable %1',
	PROCEDURE_ALREADY_EXISTS: 'Ya existe un procedimiento con el nombre "%1".',

	// Logic block messages
	CONTROLS_IF_MSG_IF: 'si',
	CONTROLS_IF_MSG_THEN: 'entonces',
	CONTROLS_IF_MSG_ELSE: 'sino',
	CONTROLS_IF_MSG_ELSEIF: 'sino si',
	CONTROLS_IF_IF_TITLE_IF: 'si',
	CONTROLS_IF_ELSEIF_TITLE_ELSEIF: 'sino si',
	CONTROLS_IF_ELSE_TITLE_ELSE: 'sino',
	LOGIC_COMPARE_TOOLTIP_EQ: 'Retorna verdadero si ambas entradas son iguales.',
	LOGIC_COMPARE_TOOLTIP_NEQ: 'Retorna verdadero si ambas entradas no son iguales.',
	LOGIC_COMPARE_TOOLTIP_LT: 'Retorna verdadero si la primera entrada es menor que la segunda entrada.',
	LOGIC_COMPARE_TOOLTIP_LTE: 'Retorna verdadero si la primera entrada es menor o igual a la segunda entrada.',
	LOGIC_COMPARE_TOOLTIP_GT: 'Retorna verdadero si la primera entrada es mayor que la segunda entrada.',
	LOGIC_COMPARE_TOOLTIP_GTE: 'Retorna verdadero si la primera entrada es mayor o igual a la segunda entrada.',
	LOGIC_OPERATION_AND: 'y',
	LOGIC_OPERATION_OR: 'o',
	LOGIC_NEGATE_TITLE: 'no %1',
	LOGIC_BOOLEAN_TRUE: 'verdadero',
	LOGIC_BOOLEAN_FALSE: 'falso',
	LOGIC_NULL: 'nulo',

	// Additional Logic Block Messages
	LOGIC_COMPARE_HELPURL: 'https://es.wikipedia.org/wiki/Desigualdad_(matemáticas)',
	LOGIC_NEGATE_HELPURL: 'https://github.com/google/blockly/wiki/Logic#not',
	LOGIC_NEGATE_TOOLTIP: 'Retorna verdadero si la entrada es falsa. Retorna falso si la entrada es verdadera.',
	LOGIC_OPERATION_TOOLTIP_AND: 'Retorna verdadero si ambas entradas son verdaderas.',
	LOGIC_OPERATION_TOOLTIP_OR: 'Retorna verdadero si al menos una de las entradas es verdadera.',
	LOGIC_BOOLEAN_TOOLTIP: 'Retorna verdadero o falso.',

	// Loop block messages
	CONTROLS_REPEAT_TITLE: 'repetir %1 veces',
	CONTROLS_REPEAT_INPUT_DO: 'hacer',
	CONTROLS_FOREVER_TITLE: 'repetir para siempre',
	CONTROLS_FOREVER_TOOLTIP: 'Sigue repitiendo las instrucciones de dentro hasta que el programa se detenga.',
	CONTROLS_WHILEUNTIL_OPERATOR_WHILE: 'repetir mientras',
	CONTROLS_WHILEUNTIL_OPERATOR_UNTIL: 'repetir hasta que',
	CONTROLS_FOR_TITLE: 'contar con %1 desde %2 hasta %3 en incrementos de %4',
	CONTROLS_FLOW_STATEMENTS_OPERATOR_BREAK: 'salir del bucle',
	CONTROLS_FLOW_STATEMENTS_OPERATOR_CONTINUE: 'continuar con la siguiente iteración',
	CONTROLS_REPEAT_TOOLTIP: 'Repite algunas instrucciones varias veces.',
	CONTROLS_WHILEUNTIL_TOOLTIP_WHILE: 'Mientras un valor sea verdadero, ejecuta algunas instrucciones.',
	CONTROLS_WHILEUNTIL_TOOLTIP_UNTIL: 'Mientras un valor sea falso, ejecuta algunas instrucciones.',
	CONTROLS_FOR_TOOLTIP: 'Cuenta desde el número inicial hasta el número final con el intervalo especificado.',
	CONTROLS_FLOW_STATEMENTS_WARNING: 'Advertencia: Este bloque solo puede ser usado dentro de un bucle.',

	// Math block messages
	MATH_NUMBER_HELPURL: 'https://es.wikipedia.org/wiki/Número',
	MATH_NUMBER_TOOLTIP: 'Un número.',
	MATH_ARITHMETIC_OPERATOR_ADD: '+',
	MATH_ARITHMETIC_OPERATOR_MINUS: '-',
	MATH_ARITHMETIC_OPERATOR_MULTIPLY: '×',
	MATH_ARITHMETIC_OPERATOR_DIVIDE: '÷',
	MATH_ARITHMETIC_OPERATOR_POWER: '^',
	MATH_SINGLE_OPERATOR_ROOT: 'raíz cuadrada',
	MATH_SINGLE_OPERATOR_ABSOLUTE: 'valor absoluto',
	MATH_IS_EVEN: 'es par',
	MATH_IS_ODD: 'es impar',
	MATH_IS_PRIME: 'es primo',
	MATH_IS_WHOLE: 'es entero',
	MATH_IS_POSITIVE: 'es positivo',
	MATH_IS_NEGATIVE: 'es negativo',
	MATH_ARITHMETIC_HELPURL: 'https://es.wikipedia.org/wiki/Aritmética',
	MATH_ARITHMETIC_TOOLTIP_ADD: 'Retorna la suma de los dos números.',
	MATH_ARITHMETIC_TOOLTIP_MINUS: 'Retorna la diferencia de los dos números.',
	MATH_ARITHMETIC_TOOLTIP_MULTIPLY: 'Retorna el producto de los dos números.',
	MATH_ARITHMETIC_TOOLTIP_DIVIDE: 'Retorna el cociente de los dos números.',
	MATH_ARITHMETIC_TOOLTIP_POWER: 'Retorna el primer número elevado a la potencia del segundo número.',

	// Text block messages
	TEXT_JOIN_TITLE_CREATEWITH: 'crear texto con',
	TEXT_CREATE_JOIN_TITLE_JOIN: 'unir',
	TEXT_LENGTH_TITLE: 'longitud de %1',
	TEXT_ISEMPTY_TITLE: '%1 está vacío',
	TEXT_INDEXOF_OPERATOR_FIRST: 'encontrar primera aparición del texto',
	TEXT_INDEXOF_OPERATOR_LAST: 'encontrar última aparición del texto',
	TEXT_CHARAT_FROM_START: 'obtener letra #',
	TEXT_CHARAT_FROM_END: 'obtener letra # desde el final',
	TEXT_CHARAT_FIRST: 'obtener primera letra',
	TEXT_CHARAT_LAST: 'obtener última letra',
	TEXT_CHARAT_RANDOM: 'obtener letra aleatoria',
	TEXT_JOIN_TOOLTIP: 'Crea un fragmento de texto uniendo cualquier número de elementos.',
	TEXT_APPEND_VARIABLE: 'elemento',
	TEXT_APPEND_TOOLTIP: 'Añade texto a la variable "%1".',
	TEXT_LENGTH_TOOLTIP: 'Retorna el número de letras (incluyendo espacios) en el texto proporcionado.',
	TEXT_ISEMPTY_TOOLTIP: 'Retorna verdadero si el texto proporcionado está vacío.',

	// List block messages
	LISTS_CREATE_EMPTY_TITLE: 'crear lista vacía',
	LISTS_CREATE_WITH_INPUT_WITH: 'crear lista con',
	LISTS_LENGTH_TITLE: 'longitud de %1',
	LISTS_ISEMPTY_TITLE: '%1 está vacía',
	LISTS_INDEXOF_FIRST: 'encontrar primera aparición del elemento',
	LISTS_INDEXOF_LAST: 'encontrar última aparición del elemento',
	LISTS_GET_INDEX_GET: 'obtener',
	LISTS_GET_INDEX_REMOVE: 'eliminar',
	LISTS_GET_INDEX_FROM_START: '#',
	LISTS_GET_INDEX_FROM_END: '# desde el final',
	LISTS_GET_INDEX_FIRST: 'primero',
	LISTS_GET_INDEX_LAST: 'último',
	LISTS_GET_INDEX_RANDOM: 'aleatorio',
	LISTS_CREATE_WITH_TOOLTIP: 'Crea una lista con cualquier número de elementos.',
	LISTS_CREATE_EMPTY_TOOLTIP: 'Retorna una lista de longitud 0, que no contiene registros de datos',
	LISTS_LENGTH_TOOLTIP: 'Retorna la longitud de una lista.',
	LISTS_ISEMPTY_TOOLTIP: 'Retorna verdadero si la lista está vacía.',

	// Variables
	VARIABLES_SET: 'establecer %1 a %2',
	VARIABLES_GET: '%1',
	VARIABLES_DEFAULT_NAME: 'elemento',

	// If conditions
	CONTROLS_IF_TOOLTIP_1: 'Si un valor es verdadero, entonces ejecuta algunas instrucciones.',
	CONTROLS_IF_TOOLTIP_2:
		'Si un valor es verdadero, entonces ejecuta el primer bloque de instrucciones. De lo contrario, ejecuta el segundo bloque de instrucciones.',
	CONTROLS_IF_TOOLTIP_3:
		'Si el primer valor es verdadero, entonces ejecuta el primer bloque de instrucciones. De lo contrario, si el segundo valor es verdadero, ejecuta el segundo bloque de instrucciones.',
	CONTROLS_IF_TOOLTIP_4:
		'Si el primer valor es verdadero, entonces ejecuta el primer bloque de instrucciones. De lo contrario, si el segundo valor es verdadero, ejecuta el segundo bloque de instrucciones. Si ninguno de los valores es verdadero, ejecuta el último bloque de instrucciones.',

	// Procedures
	PROCEDURES_DEFNORETURN_TITLE: '',
	PROCEDURES_DEFNORETURN_PROCEDURE: 'hacer algo',
	PROCEDURES_BEFORE_PARAMS: 'con:',
	PROCEDURES_CALL_BEFORE_PARAMS: 'con:',
	PROCEDURES_DEFNORETURN_DO: '',
	PROCEDURES_DEFNORETURN_TOOLTIP: 'Crea una función sin salida.',
	PROCEDURES_DEFRETURN_RETURN: 'retornar',
	PROCEDURES_DEFRETURN_TOOLTIP: 'Crea una función con una salida.',
	PROCEDURES_DEFRETURN_COMMENT: 'Describe esta función...',
	PROCEDURES_DEFRETURN_PROCEDURE: 'hacer algo con retorno',
	PROCEDURES_DEFRETURN_DO: '',
	PROCEDURES_CALLNORETURN_HELPURL: 'https://es.wikipedia.org/wiki/Subrutina',
	PROCEDURES_CALLNORETURN_TOOLTIP: 'Ejecuta la función definida por el usuario.',
	PROCEDURES_CALLRETURN_HELPURL: 'https://es.wikipedia.org/wiki/Subrutina',
	PROCEDURES_CALLRETURN_TOOLTIP: 'Ejecuta la función definida por el usuario y usa su salida.',

	// Seven Segment Display
	SEVEN_SEGMENT_DISPLAY: 'Pantalla de Siete Segmentos',
	SEVEN_SEGMENT_COMMON_CATHODE: 'Cátodo Común',
	SEVEN_SEGMENT_COMMON_ANODE: 'Ánodo Común',
	SEVEN_SEGMENT_NUMBER: 'Número (0-9):',
	SEVEN_SEGMENT_DECIMAL_POINT: 'Punto Decimal',
	SEVEN_SEGMENT_TOOLTIP: 'Muestra un número (0-9) en una pantalla de siete segmentos con punto decimal opcional.',
	// Seven Segment Display Pins
	SEVEN_SEGMENT_PINS_SET: 'Configurar pines de pantalla de siete segmentos',
	SEVEN_SEGMENT_PINS_TOOLTIP: 'Configura pines para cada segmento (A-G) y punto decimal (DP) de la pantalla de siete segmentos.',

	// Pixetto Smart Camera
	PIXETTO_INIT: 'Inicializar Cámara Inteligente Pixetto',
	PIXETTO_RX_PIN: 'Pin RX',
	PIXETTO_TX_PIN: 'Pin TX',
	PIXETTO_IS_DETECTED: 'Pixetto Objeto Detectado',
	PIXETTO_GET_TYPE_ID: 'Pixetto Obtener ID de Tipo',
	PIXETTO_GET_FUNC_ID: 'Pixetto Obtener ID de Función',
	PIXETTO_COLOR_DETECT: 'Pixetto Detectar Color',
	PIXETTO_SHAPE_DETECT: 'Pixetto Detectar Forma',
	PIXETTO_FACE_DETECT: 'Pixetto Detectar Rostro',
	PIXETTO_APRILTAG_DETECT: 'Pixetto Detectar AprilTag',
	PIXETTO_NEURAL_NETWORK: 'Pixetto Reconocimiento de Red Neuronal',
	PIXETTO_HANDWRITTEN_DIGIT: 'Pixetto Reconocer Dígito Manuscrito',
	PIXETTO_GET_POSITION: 'Pixetto Obtener Objeto Detectado',
	PIXETTO_ROAD_DETECT: 'Pixetto Detección de Carretera',
	PIXETTO_SET_MODE: 'Configurar Modo de Función Pixetto',
	PIXETTO_COLOR: 'Color',
	PIXETTO_SHAPE: 'Forma',
	PIXETTO_MODE: 'Modo',
	PIXETTO_TAG_ID: 'ID de Etiqueta',
	PIXETTO_CLASS_ID: 'ID de Clase',
	PIXETTO_DIGIT: 'Dígito',
	PIXETTO_COORDINATE: 'Coordenada',
	PIXETTO_ROAD_INFO: 'Información',
	// Vision Sensors Category Labels
	PIXETTO_LABEL: 'Pixetto',
	HUSKYLENS_LABEL: 'HUSKYLENS',

	// Pixetto Tooltips
	PIXETTO_INIT_TOOLTIP: 'Inicializar la cámara inteligente Pixetto y configurar los pines de comunicación UART',
	PIXETTO_IS_DETECTED_TOOLTIP: 'Detectar si Pixetto detecta algún objeto',
	PIXETTO_GET_TYPE_ID_TOOLTIP: 'Obtener el ID del tipo de objeto detectado por Pixetto (color, forma, etc.)',
	PIXETTO_GET_FUNC_ID_TOOLTIP: 'Obtener el ID de la función actualmente utilizada por Pixetto (detección de color, forma, etc.)',
	PIXETTO_COLOR_DETECT_TOOLTIP: 'Detectar si Pixetto detecta un objeto del color especificado',
	PIXETTO_SHAPE_DETECT_TOOLTIP: 'Detectar si Pixetto detecta un objeto de la forma especificada',
	PIXETTO_FACE_DETECT_TOOLTIP: 'Detectar si Pixetto detecta una cara',
	PIXETTO_APRILTAG_DETECT_TOOLTIP: 'Detectar si Pixetto detecta un AprilTag con el ID especificado',
	PIXETTO_NEURAL_NETWORK_TOOLTIP: 'Detectar si la red neuronal de Pixetto reconoce un objeto de la clase especificada',
	PIXETTO_HANDWRITTEN_DIGIT_TOOLTIP: 'Detectar si Pixetto reconoce un dígito manuscrito especificado',
	PIXETTO_GET_POSITION_TOOLTIP: 'Obtener información de posición o tamaño del objeto detectado por Pixetto',
	PIXETTO_ROAD_DETECT_TOOLTIP: 'Obtener información relacionada con la detección de carreteras de Pixetto',
	PIXETTO_SET_MODE_TOOLTIP: 'Configurar el modo funcional de la cámara inteligente Pixetto',

	// Pixetto Color Options
	PIXETTO_COLOR_RED: 'Rojo',
	PIXETTO_COLOR_BLUE: 'Azul',
	PIXETTO_COLOR_GREEN: 'Verde',
	PIXETTO_COLOR_YELLOW: 'Amarillo',
	PIXETTO_COLOR_ORANGE: 'Naranja',
	PIXETTO_COLOR_PURPLE: 'Púrpura',
	PIXETTO_COLOR_BLACK: 'Negro',
	PIXETTO_COLOR_WHITE: 'Blanco',

	// Pixetto Shape Options
	PIXETTO_SHAPE_TRIANGLE: 'Triángulo',
	PIXETTO_SHAPE_RECTANGLE: 'Rectángulo',
	PIXETTO_SHAPE_PENTAGON: 'Pentágono',
	PIXETTO_SHAPE_HEXAGON: 'Hexágono',
	PIXETTO_SHAPE_CIRCLE: 'Círculo',

	// Pixetto Position Options
	PIXETTO_POSITION_X: 'Coordenada X',
	PIXETTO_POSITION_Y: 'Coordenada Y',
	PIXETTO_POSITION_WIDTH: 'Ancho',
	PIXETTO_POSITION_HEIGHT: 'Alto',

	// Pixetto Road Info Options
	PIXETTO_ROAD_CENTER_X: 'Centro X',
	PIXETTO_ROAD_CENTER_Y: 'Centro Y',
	PIXETTO_ROAD_LEFT_X: 'Límite Izquierdo X',
	PIXETTO_ROAD_RIGHT_X: 'Límite Derecho X',

	// Pixetto Mode Options
	PIXETTO_MODE_COLOR_DETECTION: 'Detección de Color',
	PIXETTO_MODE_SHAPE_DETECTION: 'Detección de Forma',
	PIXETTO_MODE_FACE_DETECTION: 'Detección de Cara',
	PIXETTO_MODE_APRILTAG_DETECTION: 'Detección de AprilTag',
	PIXETTO_MODE_NEURAL_NETWORK: 'Red Neuronal',
	PIXETTO_MODE_HANDWRITTEN_DIGIT: 'Dígito Manuscrito',
	PIXETTO_MODE_ROAD_DETECTION: 'Detección de Carretera',
	PIXETTO_MODE_BALL_DETECTION: 'Detección de Pelota',
	PIXETTO_MODE_TEMPLATE_MATCHING: 'Coincidencia de Plantilla',

	// HUSKYLENS Smart Camera
	HUSKYLENS_INIT_I2C: 'Inicializar HUSKYLENS (I2C)',
	HUSKYLENS_INIT_UART: 'Inicializar HUSKYLENS (UART)',
	HUSKYLENS_RX_PIN: 'Conectar a HuskyLens TX →',
	HUSKYLENS_TX_PIN: 'Conectar a HuskyLens RX →',
	HUSKYLENS_SET_ALGORITHM: 'Configurar Algoritmo HUSKYLENS',
	HUSKYLENS_ALGORITHM_FACE_RECOGNITION: 'Reconocimiento Facial',
	HUSKYLENS_ALGORITHM_OBJECT_TRACKING: 'Seguimiento de Objeto',
	HUSKYLENS_ALGORITHM_OBJECT_RECOGNITION: 'Reconocimiento de Objeto',
	HUSKYLENS_ALGORITHM_LINE_TRACKING: 'Seguimiento de Línea',
	HUSKYLENS_ALGORITHM_COLOR_RECOGNITION: 'Reconocimiento de Color',
	HUSKYLENS_ALGORITHM_TAG_RECOGNITION: 'Reconocimiento de Etiqueta',
	HUSKYLENS_ALGORITHM_OBJECT_CLASSIFICATION: 'Clasificación de Objeto',
	HUSKYLENS_REQUEST: 'Solicitar Datos HUSKYLENS',
	HUSKYLENS_IS_LEARNED: 'HUSKYLENS Ha Aprendido',
	HUSKYLENS_COUNT_BLOCKS: 'Número de Bloques HUSKYLENS',
	HUSKYLENS_COUNT_ARROWS: 'Número de Flechas HUSKYLENS',
	HUSKYLENS_GET_BLOCK_INFO: 'Obtener Bloque',
	HUSKYLENS_GET_ARROW_INFO: 'Obtener Flecha',
	HUSKYLENS_BLOCK_INFO_TYPE: ' ',
	HUSKYLENS_ARROW_INFO_TYPE: ' ',
	HUSKYLENS_X_CENTER: 'Centro X',
	HUSKYLENS_Y_CENTER: 'Centro Y',
	HUSKYLENS_WIDTH: 'Ancho',
	HUSKYLENS_HEIGHT: 'Alto',
	HUSKYLENS_ID: 'ID',
	HUSKYLENS_X_ORIGIN: 'Origen X',
	HUSKYLENS_Y_ORIGIN: 'Origen Y',
	HUSKYLENS_X_TARGET: 'Destino X',
	HUSKYLENS_Y_TARGET: 'Destino Y',
	HUSKYLENS_LEARN: 'HUSKYLENS Aprender ID',
	HUSKYLENS_FORGET: 'HUSKYLENS Olvidar Todo',

	// HuskyLens ID-Based Bloques
	HUSKYLENS_BY_ID_LABEL: 'Consultar por ID',
	HUSKYLENS_REQUEST_BLOCKS_ID: 'solicitar bloques HUSKYLENS con ID',
	HUSKYLENS_REQUEST_BLOCKS_ID_SUFFIX: '',
	HUSKYLENS_REQUEST_BLOCKS_ID_TOOLTIP: 'Solicitar solo bloques con ID específico para mayor eficiencia',
	HUSKYLENS_COUNT_BLOCKS_ID: 'cantidad de bloques HUSKYLENS con ID',
	HUSKYLENS_COUNT_BLOCKS_ID_SUFFIX: '',
	HUSKYLENS_COUNT_BLOCKS_ID_TOOLTIP: 'Obtener la cantidad de bloques con ID específico',
	HUSKYLENS_GET_BLOCK_ID: 'obtener bloque con ID',
	HUSKYLENS_GET_BLOCK_ID_INDEX: 'índice',
	HUSKYLENS_GET_BLOCK_ID_INDEX_SUFFIX: '',
	HUSKYLENS_GET_BLOCK_ID_TOOLTIP: 'Obtener posición, tamaño o ID de un bloque con ID específico',

	// HUSKYLENS Tooltips
	HUSKYLENS_INIT_I2C_TOOLTIP: 'Inicializar cámara inteligente HUSKYLENS usando I2C',
	HUSKYLENS_INIT_UART_TOOLTIP: 'Inicializar cámara inteligente HUSKYLENS usando UART, configurar pines RX/TX',
	HUSKYLENS_SET_ALGORITHM_TOOLTIP: 'Configurar el algoritmo de reconocimiento usado por HUSKYLENS',
	HUSKYLENS_REQUEST_TOOLTIP: 'Solicitar los últimos resultados de reconocimiento de HUSKYLENS',
	HUSKYLENS_IS_LEARNED_TOOLTIP: 'Verificar si HUSKYLENS ha aprendido algo',
	HUSKYLENS_COUNT_BLOCKS_TOOLTIP: 'Obtener el número de bloques detectados por HUSKYLENS',
	HUSKYLENS_COUNT_ARROWS_TOOLTIP: 'Obtener el número de flechas detectadas por HUSKYLENS',
	HUSKYLENS_GET_BLOCK_INFO_TOOLTIP: 'Obtener información del bloque especificado (posición, tamaño o ID)',
	HUSKYLENS_GET_ARROW_INFO_TOOLTIP: 'Obtener información de flecha especificada (origen, destino o ID)',
	HUSKYLENS_LEARN_TOOLTIP: 'Hacer que HUSKYLENS aprenda objeto con ID especificado (solo modo clasificación de objeto)',
	HUSKYLENS_FORGET_TOOLTIP: 'Borrar todos los objetos aprendidos por HUSKYLENS (solo modo clasificación de objeto)',
	HUSKYLENS_I2C_PIN_HINT: 'Cableado: ',
	HUSKYLENS_UART_PIN_HINT: 'Pines recomendados: ',
	HUSKYLENS_UART_ANY_DIGITAL: 'Cualquier pin digital',
	// ESP32 PWM Setup
	ESP32_PWM_SETUP: 'Configuración PWM ESP32',
	ESP32_PWM_FREQUENCY: 'Frecuencia',
	ESP32_PWM_RESOLUTION: 'Resolución',
	ESP32_PWM_FREQUENCY_TOOLTIP:
		'Establecer frecuencia PWM, rango 1-80000 Hz. Alta frecuencia para chips controladores de motor (20-75KHz)',
	ESP32_PWM_RESOLUTION_TOOLTIP: 'Establecer resolución PWM, afecta la precisión de salida. Nota: frecuencia × 2^resolución ≤ 80.000.000',
	ESP32_PWM_RESOLUTION_8BIT: '8 bits (0-255)',
	ESP32_PWM_RESOLUTION_10BIT: '10 bits (0-1023)',
	ESP32_PWM_RESOLUTION_12BIT: '12 bits (0-4095)',
	ESP32_PWM_RESOLUTION_13BIT: '13 bits (0-8191)',
	ESP32_PWM_RESOLUTION_14BIT: '14 bits (0-16383)',
	ESP32_PWM_RESOLUTION_15BIT: '15 bits (0-32767)',
	ESP32_PWM_RESOLUTION_16BIT: '16 bits (0-65535)',
	// Safety Guard (Project Safety Protection)
	SAFETY_WARNING_BODY_NO_TYPE:
		'Esta carpeta aún no tiene archivos de Blockly. Si continúas, se crearán aquí una carpeta blockly/ y sus archivos. Elige Cancelar para no hacer cambios.',
	SAFETY_WARNING_BODY_WITH_TYPE:
		'Esta carpeta ya contiene archivos de {0}. Si continúas, también se crearán aquí una carpeta blockly/ y sus archivos. Elige Cancelar para no hacer cambios.',
	BUTTON_CONTINUE: '¡Sí, vamos!',
	BUTTON_CANCEL: 'Cancelar',
	BUTTON_SUPPRESS: 'No preguntar',
	SAFETY_GUARD_CANCELLED: '¡No te preocupes! No hicimos ningún cambio.',
	SAFETY_GUARD_SUPPRESSED: '¡Entendido! No te preguntaremos de nuevo sobre esto.',

	// Communication Category (WiFi/MQTT)
	CATEGORY_COMMUNICATION: 'Comunicación',
	ESP32_WIFI_LABEL: 'WiFi',
	ESP32_MQTT_LABEL: 'MQTT',
	ESP32_WIFI_CONNECT: 'WiFi Conectar',
	ESP32_WIFI_CONNECT_SSID: 'SSID',
	ESP32_WIFI_CONNECT_PASSWORD: 'Contraseña',
	ESP32_WIFI_CONNECT_TOOLTIP: 'Conectar a red WiFi (tiempo de espera 10 segundos)',
	ESP32_WIFI_DISCONNECT: 'WiFi Desconectar',
	ESP32_WIFI_DISCONNECT_TOOLTIP: 'Desconectar WiFi',
	ESP32_WIFI_STATUS: '¿WiFi Conectado?',
	ESP32_WIFI_STATUS_TOOLTIP: 'Devuelve el estado de conexión WiFi',
	ESP32_WIFI_GET_IP: 'Dirección IP WiFi',
	ESP32_WIFI_GET_IP_TOOLTIP: 'Obtener dirección IP actual',
	ESP32_WIFI_SCAN: 'Escanear Redes WiFi',
	ESP32_WIFI_SCAN_TOOLTIP: 'Escanear y devolver cantidad de redes WiFi cercanas',
	ESP32_WIFI_GET_SSID: 'Obtener SSID WiFi',
	ESP32_WIFI_GET_SSID_INDEX: 'índice',
	ESP32_WIFI_GET_SSID_TOOLTIP: 'Obtener nombre de WiFi en índice especificado',
	ESP32_WIFI_GET_RSSI: 'Obtener Intensidad de Señal WiFi',
	ESP32_WIFI_GET_RSSI_INDEX: 'índice',
	ESP32_WIFI_GET_RSSI_TOOLTIP: 'Obtener intensidad de señal en índice especificado (dBm)',
	ESP32_MQTT_SETUP: 'Configuración MQTT',
	ESP32_MQTT_SETUP_SERVER: 'Servidor',
	ESP32_MQTT_SETUP_PORT: 'Puerto',
	ESP32_MQTT_SETUP_CLIENT_ID: 'ID de Cliente',
	ESP32_MQTT_SETUP_TOOLTIP: 'Configurar parámetros de conexión del servidor MQTT',
	ESP32_MQTT_CONNECT: 'MQTT Conectar',
	ESP32_MQTT_CONNECT_USERNAME: 'Usuario',
	ESP32_MQTT_CONNECT_PASSWORD: 'Contraseña',
	ESP32_MQTT_CONNECT_TOOLTIP: 'Conectar al servidor MQTT',
	ESP32_MQTT_PUBLISH: 'MQTT Publicar',
	ESP32_MQTT_PUBLISH_TOPIC: 'Tema',
	ESP32_MQTT_PUBLISH_MESSAGE: 'Mensaje',
	ESP32_MQTT_PUBLISH_TOOLTIP: 'Publicar mensaje en tema especificado',
	ESP32_MQTT_SUBSCRIBE: 'MQTT Suscribir',
	ESP32_MQTT_SUBSCRIBE_TOPIC: 'Tema',
	ESP32_MQTT_SUBSCRIBE_TOOLTIP: 'Suscribirse a mensajes del tema especificado',
	ESP32_MQTT_LOOP: 'MQTT Procesar Mensajes',
	ESP32_MQTT_LOOP_TOOLTIP: 'Mantener conexión y procesar mensajes recibidos (poner en loop)',
	ESP32_MQTT_GET_TOPIC: 'MQTT Último Tema',
	ESP32_MQTT_GET_TOPIC_TOOLTIP: 'Obtener tema del mensaje más reciente',
	ESP32_MQTT_GET_MESSAGE: 'MQTT Último Mensaje',
	ESP32_MQTT_GET_MESSAGE_TOOLTIP: 'Obtener contenido del mensaje más reciente',
	ESP32_MQTT_STATUS: 'MQTT Conectado',
	ESP32_MQTT_STATUS_TOOLTIP: 'Verificar si está conectado al servidor MQTT',
	TEXT_TO_NUMBER: 'Texto a Número',
	TEXT_TO_NUMBER_INT: 'Entero',
	TEXT_TO_NUMBER_FLOAT: 'Decimal',
	TEXT_TO_NUMBER_TOOLTIP: 'Convertir texto a número (entrada inválida devuelve 0)',

	// To String Block
	TO_STRING: 'A Texto',
	TO_STRING_TOOLTIP: 'Convertir número o booleano a texto',

	// ESP32 Only Block Warning
	ESP32_ONLY_BLOCK_WARNING: 'Este bloque solo soporta placas ESP32',

	// Quick Backup Toast Messages
	BACKUP_QUICK_SAVE_SUCCESS: 'Copia guardada: {0}',
	BACKUP_QUICK_SAVE_EMPTY: 'El espacio de trabajo está vacío, no se necesita copia de seguridad',
	BACKUP_QUICK_SAVE_COOLDOWN: 'Por favor espere, la copia de seguridad acaba de completarse',
	MAIN_BLOCK_DUPLICATE_WARNING: 'Se detectaron varios bloques de programa principal. Elimina los bloques sobrantes.',

	// Board Switch Warning
	BOARD_SWITCH_WARNING_TITLE: 'Cambiar tipo de placa',
	BOARD_SWITCH_WARNING_MESSAGE:
		'Cambiar a un tipo de placa diferente borrará el espacio de trabajo actual.\nSu trabajo se respaldará automáticamente primero.\n\n¿Desea continuar?',

	// CyberBrick Board
	BOARD_CYBERBRICK: 'CyberBrick',

	// CyberBrick Category
	CATEGORY_CYBERBRICK_CORE: 'CyberBrick',
	CATEGORY_CYBERBRICK_LED: 'LED',
	CATEGORY_CYBERBRICK_GPIO: 'GPIO',
	CATEGORY_CYBERBRICK_TIME: 'Tiempo',

	// CyberBrick Main Block
	CYBERBRICK_MAIN: 'Programa principal',
	CYBERBRICK_MAIN_TOOLTIP: 'Punto de entrada del programa principal de CyberBrick. Todo el código debe colocarse dentro de este bloque.',

	// CyberBrick LED Blocks
	CYBERBRICK_LED_SET_COLOR: 'Establecer color LED',
	CYBERBRICK_LED_SET_COLOR_PREFIX: 'Establecer color del LED integrado',
	CYBERBRICK_LED_RED: 'Rojo',
	CYBERBRICK_LED_GREEN: 'Verde',
	CYBERBRICK_LED_BLUE: 'Azul',
	CYBERBRICK_LED_SET_COLOR_TOOLTIP: 'Establecer color del LED integrado (GPIO8) usando valores RGB (0-255)',
	CYBERBRICK_LED_OFF: 'Apagar LED',
	CYBERBRICK_LED_OFF_TOOLTIP: 'Apagar el LED integrado',
	CYBERBRICK_LED_DIGITAL_PREFIX: 'Establecer LED integrado (digital)',
	CYBERBRICK_LED_DIGITAL_TOOLTIP: 'Activar o desactivar los canales del LED integrado',
	CYBERBRICK_LED_DIGITAL_ON: 'ENCENDIDO',
	CYBERBRICK_LED_DIGITAL_OFF: 'APAGADO',

	// CyberBrick GPIO Blocks
	CYBERBRICK_GPIO_SET: 'Establecer GPIO',
	CYBERBRICK_GPIO_SET_PREFIX: 'GPIO',
	CYBERBRICK_GPIO_SET_TO: 'a',
	CYBERBRICK_GPIO_PIN: 'Pin',
	CYBERBRICK_GPIO_VALUE: 'Valor',
	CYBERBRICK_GPIO_HIGH: 'ALTO',
	CYBERBRICK_GPIO_LOW: 'BAJO',
	CYBERBRICK_GPIO_SET_TOOLTIP: 'Establecer pin GPIO a ALTO o BAJO',
	CYBERBRICK_GPIO_READ: 'Leer GPIO',
	CYBERBRICK_GPIO_READ_TOOLTIP: 'Leer valor digital del pin GPIO (devuelve 0 o 1)',

	// CyberBrick Time Blocks
	CYBERBRICK_DELAY_MS: 'Retardo (ms)',
	CYBERBRICK_DELAY_MS_PREFIX: 'Retardo',
	CYBERBRICK_DELAY_MS_SUFFIX: 'ms',
	CYBERBRICK_DELAY_MS_TOOLTIP: 'Pausar ejecución del programa por milisegundos especificados',
	CYBERBRICK_DELAY_S: 'Retardo (s)',
	CYBERBRICK_DELAY_S_PREFIX: 'Retardo',
	CYBERBRICK_DELAY_S_SUFFIX: 'segundos',
	CYBERBRICK_DELAY_S_TOOLTIP: 'Pausar ejecución del programa por segundos especificados',
	CYBERBRICK_TICKS_MS: 'Obtener milisegundos actuales',
	CYBERBRICK_TICKS_MS_TOOLTIP: 'Obtener el contador actual de milisegundos',
	CYBERBRICK_TICKS_DIFF_PREFIX: 'Diferencia de tiempo',
	CYBERBRICK_TICKS_DIFF_NOW: 'ahora',
	CYBERBRICK_TICKS_DIFF_START: 'inicio',
	CYBERBRICK_TICKS_DIFF_TOOLTIP: 'Calcular los milisegundos entre ahora y el inicio',

	// CyberBrick WiFi Blocks
	CATEGORY_CYBERBRICK_WIFI: 'WiFi',
	CYBERBRICK_WIFI_CONNECT: 'Conectar WiFi',
	CYBERBRICK_WIFI_SSID: 'SSID',
	CYBERBRICK_WIFI_PASSWORD: 'Contraseña',
	CYBERBRICK_WIFI_CONNECT_TOOLTIP: 'Conectar a la red WiFi especificada',
	CYBERBRICK_WIFI_DISCONNECT: 'Desconectar WiFi',
	CYBERBRICK_WIFI_DISCONNECT_TOOLTIP: 'Desconectar de la red WiFi actual',
	CYBERBRICK_WIFI_IS_CONNECTED: '¿WiFi conectado?',
	CYBERBRICK_WIFI_IS_CONNECTED_TOOLTIP: 'Verificar si WiFi está conectado',
	CYBERBRICK_WIFI_GET_IP: 'Obtener dirección IP',
	CYBERBRICK_WIFI_GET_IP_TOOLTIP: 'Obtener dirección IP actual',

	// CyberBrick Upload
	UPLOAD_BUTTON_TITLE: 'Subir a CyberBrick',
	UPLOAD_BUTTON_DISABLED_TITLE: 'Guarde el espacio de trabajo primero para habilitar la carga',
	UPLOAD_STARTING: 'Iniciando carga...',
	UPLOAD_SUCCESS: '¡Carga exitosa!',
	UPLOAD_FAILED: 'Carga fallida: {0}',
	UPLOAD_NO_PORT: 'No se encontró dispositivo CyberBrick',
	UPLOAD_IN_PROGRESS: 'Cargando...',
	UPLOAD_EMPTY_WORKSPACE: 'El espacio de trabajo está vacío, agregue bloques primero',
	UPLOAD_NO_CODE: 'No se puede generar código',

	// Upload progress stages
	UPLOAD_STAGE_PREPARING: 'Preparando',
	UPLOAD_STAGE_CHECKING: 'Verificando herramientas',
	UPLOAD_STAGE_INSTALLING: 'Instalando herramientas',
	UPLOAD_STAGE_CONNECTING: 'Conectando dispositivo',
	UPLOAD_STAGE_RESETTING: 'Reiniciando dispositivo',
	UPLOAD_STAGE_BACKUP: 'Respaldando',
	UPLOAD_STAGE_UPLOADING: 'Cargando',
	UPLOAD_STAGE_RESTARTING: 'Reiniciando dispositivo',
	UPLOAD_STAGE_COMPLETED: 'Completado',

	// Upload error messages
	ERROR_UPLOAD_BOARD_UNSUPPORTED: 'Solo se admite la placa CyberBrick',
	ERROR_UPLOAD_CODE_EMPTY: 'El código no puede estar vacío',
	ERROR_UPLOAD_NO_PYTHON: 'No se encontró el entorno Python de PlatformIO. Por favor, instale PlatformIO primero.',
	ERROR_UPLOAD_MPREMOTE_FAILED: 'Fallo en la instalación de mpremote',
	ERROR_UPLOAD_DEVICE_NOT_FOUND: 'No se encontró el dispositivo CyberBrick. Por favor, asegúrese de que esté conectado.',
	ERROR_UPLOAD_RESET_FAILED: 'Error al reiniciar el dispositivo',
	ERROR_UPLOAD_UPLOAD_FAILED: 'Error al cargar el programa',
	ERROR_UPLOAD_RESTART_FAILED: 'Error al reiniciar el dispositivo',

	// Arduino Upload Button
	UPLOAD_BUTTON_TITLE_ARDUINO: 'Compilar y Cargar',
	UPLOAD_SELECT_BOARD: 'Por favor seleccione una placa primero',

	// Arduino Upload Stages
	ARDUINO_STAGE_SYNCING: 'Sincronizando configuración',
	ARDUINO_STAGE_SAVING: 'Guardando espacio de trabajo',
	ARDUINO_STAGE_CHECKING: 'Verificando compilador',
	ARDUINO_STAGE_DETECTING: 'Detectando placa',
	ARDUINO_STAGE_COMPILING: 'Compilando',
	ARDUINO_STAGE_UPLOADING: 'Cargando',

	// Arduino Upload Results
	ARDUINO_COMPILE_SUCCESS: '¡Compilación exitosa!',
	ARDUINO_UPLOAD_SUCCESS: '¡Carga exitosa!',

	// Arduino Upload Errors
	ERROR_ARDUINO_PIO_NOT_FOUND: 'PlatformIO CLI no encontrado. Por favor instale PlatformIO primero.',
	ERROR_ARDUINO_COMPILE_FAILED: 'Fallo en la compilación',
	ERROR_ARDUINO_UPLOAD_FAILED: 'Fallo en la carga',
	ERROR_ARDUINO_NO_WORKSPACE: 'Por favor abra una carpeta de proyecto primero',
	ERROR_ARDUINO_TIMEOUT: 'La operación expiró',
	ERROR_ARDUINO_NO_DEVICE: 'No se detectó ningún dispositivo. Conecte su placa.',
	ERROR_ARDUINO_PORT_BUSY: 'El puerto está siendo utilizado por otro programa. Cierre otros monitores serie.',
	ERROR_ARDUINO_DEVICE_DISCONNECT: 'El dispositivo se desconectó durante la carga.',
	ERROR_ARDUINO_UPLOAD_TIMEOUT: 'Tiempo de carga agotado. Verifique su conexión.',
	ERROR_ARDUINO_UPLOAD_CONNECTION: 'Error en la carga. Verifique la conexión del dispositivo.',

	// Backup messages
	BACKUP_CONFIRM_DELETE: '¿Está seguro de que desea eliminar la copia de seguridad "{0}"?',
	BACKUP_CONFIRM_RESTORE:
		'¿Está seguro de que desea restaurar la copia de seguridad "{0}"? Esto sobrescribirá el espacio de trabajo actual.',
	BACKUP_ERROR_NOT_FOUND: 'Copia de seguridad "{0}" no encontrada',
	BACKUP_ERROR_CREATE_FAILED: 'Error al crear la copia de seguridad: {0}',
	BACKUP_ERROR_DELETE_FAILED: 'Error al eliminar la copia de seguridad: {0}',
	BACKUP_ERROR_RESTORE_FAILED: 'Error al restaurar la copia de seguridad: {0}',
	BACKUP_ERROR_PREVIEW_FAILED: 'Error al previsualizar la copia de seguridad: {0}',
	BACKUP_ERROR_NAME_NOT_SPECIFIED: 'Nombre de copia de seguridad no especificado',
	BACKUP_ERROR_MAIN_NOT_FOUND: 'No se puede encontrar el archivo main.json',
	BACKUP_ERROR_UPDATE_SETTINGS_FAILED: 'Error al actualizar la configuración de copia de seguridad automática',

	// Button labels
	BUTTON_DELETE: 'Eliminar',
	BUTTON_RESTORE: 'Restaurar',

	// Error messages
	ERROR_PROCESSING_MESSAGE: 'Error al procesar el mensaje: {0}',
	ERROR_SETTINGS_UPDATE_FAILED: 'Error al actualizar la configuración',
	ERROR_RELOAD_WORKSPACE_FAILED: 'Error al recargar el espacio de trabajo: {0}',
	ERROR_OPEN_PROJECT_FOLDER_FIRST: 'Por favor, abra una carpeta de proyecto primero',

	// Info messages
	INFO_NO_BACKUPS_TO_PREVIEW: 'No hay archivos de copia de seguridad para previsualizar',

	// Dialog messages
	DIALOG_SELECT_BACKUP_TITLE: 'Seleccionar archivo de copia de seguridad para previsualizar',
	DIALOG_BACKUP_FILES_LABEL: 'Archivos de copia de seguridad',

	// Placa de extensión X11
	CATEGORY_X11: 'Extensión X11',
	X11_LABEL_SERVOS: 'Servos',
	X11_LABEL_MOTORS: 'Motores',
	X11_LABEL_LEDS: 'LEDs',

	// Bloques de servo 180° X11
	X11_SERVO_180_ANGLE_PREFIX: 'Establecer servo',
	X11_SERVO_180_ANGLE_SUFFIX: 'ángulo',
	X11_SERVO_180_ANGLE_TOOLTIP: 'Establecer ángulo del servo 180° (0-180 grados)',

	// Bloques de servo 360° X11
	X11_SERVO_360_SPEED_PREFIX: 'Establecer servo',
	X11_SERVO_360_SPEED_SUFFIX: 'velocidad',
	X11_SERVO_360_SPEED_TOOLTIP: 'Establecer velocidad del servo de rotación continua 360° (-100 a 100, negativo=reversa)',

	// Bloque de parada de servo X11
	X11_SERVO_STOP: 'Detener servo',
	X11_SERVO_STOP_TOOLTIP: 'Detener el servo especificado',

	// Bloques de motor X11
	X11_MOTOR_SPEED_PREFIX: 'Establecer motor',
	X11_MOTOR_SPEED_SUFFIX: 'velocidad',
	X11_MOTOR_SPEED_TOOLTIP: 'Establecer velocidad del motor DC (-2048 a 2048, negativo=reversa)',
	X11_MOTOR_STOP: 'Detener motor',
	X11_MOTOR_STOP_TOOLTIP: 'Detener el motor especificado',

	// Bloques de LED X11
	X11_LED_SET_COLOR_PREFIX: 'Tira LED',
	X11_LED_SET_COLOR_INDEX: 'índice',
	X11_LED_SET_COLOR_INDEX_SUFFIX: 'píxel',
	X11_LED_SET_COLOR_TOOLTIP: 'Establecer color de píxel de tira LED (índice 0=primer píxel, o todos)',
	X11_LED_DIGITAL_TOOLTIP: 'Establecer los estados de los canales de la tira LED X11 en ON u OFF',
	X11_LED_INDEX_ALL: 'Todos',

	// === Placa de extensión X12 Transmisor ===
	CATEGORY_X12: 'X12 Extensión',
	X12_LABEL_JOYSTICK: 'Joystick',
	X12_LABEL_BUTTON: 'Botón',

	// Bloques de Joystick X12
	X12_GET_JOYSTICK_PREFIX: 'Joystick',
	X12_GET_JOYSTICK_SUFFIX: 'valor',
	X12_GET_JOYSTICK_TOOLTIP: 'Leer valor ADC del joystick (0-4095)',
	X12_GET_JOYSTICK_MAPPED_PREFIX: 'Joystick',
	X12_GET_JOYSTICK_MAPPED_MIN: 'mapear a',
	X12_GET_JOYSTICK_MAPPED_MAX: '~',
	X12_GET_JOYSTICK_MAPPED_TOOLTIP: 'Leer joystick y mapear al rango especificado',

	// Bloques de Botón X12
	X12_IS_BUTTON_PRESSED_PREFIX: 'Botón',
	X12_IS_BUTTON_PRESSED_SUFFIX: '¿presionado?',
	X12_IS_BUTTON_PRESSED_TOOLTIP: 'Verificar si el botón está presionado',

	// === Control Remoto RC ===

	// Bloques de Inicialización RC

	// Bloques de Joystick RC

	// Bloques de Botón RC

	// Bloques de Estado RC

	// === Conexión RC ===
	CATEGORY_RC: 'Conexión RC',
	RC_LABEL_MASTER: '📡 Transmisor',
	RC_LABEL_SLAVE: '📻 Receptor',
	RC_LABEL_DATA: '📊 Lectura de datos',
	RC_LABEL_STATUS: '🔗 Estado de conexión',

	// Bloques Transmisor RC
	RC_MASTER_INIT: 'Inicializar transmisor RC',
	RC_MASTER_INIT_PAIR_ID: 'ID pareja',
	RC_MASTER_INIT_CHANNEL: 'Canal',
	RC_MASTER_INIT_TOOLTIP: 'Inicializar transmisor RC con ID pareja (1-255) y canal (1-11). Nota de canal: si ambos dispositivos se conectan al mismo Wi-Fi, RC usa el canal Wi-Fi; si ninguno está conectado, RC usa el canal configurado.',
	RC_SEND: 'Enviar datos RC',
	RC_SEND_TOOLTIP: 'Leer datos joystick/botones X12 y enviar al receptor',

	// Bloques Receptor RC
	RC_SLAVE_INIT: 'Inicializar receptor RC',
	RC_SLAVE_INIT_PAIR_ID: 'ID pareja',
	RC_SLAVE_INIT_CHANNEL: 'Canal',
	RC_SLAVE_INIT_TOOLTIP: 'Inicializar receptor RC con ID pareja (1-255) y canal (1-11). Nota de canal: si ambos dispositivos se conectan al mismo Wi-Fi, RC usa el canal Wi-Fi; si ninguno está conectado, RC usa el canal configurado.',
	RC_WAIT_CONNECTION: 'Esperar emparejamiento',
	RC_WAIT_TIMEOUT: 'tiempo límite',
	RC_WAIT_SECONDS: 'segundos',
	RC_WAIT_TOOLTIP: 'Esperar conexión transmisor, LED parpadea azul, continúa después del tiempo',
	RC_IS_CONNECTED: '¿RC conectado?',
	RC_IS_CONNECTED_TOOLTIP: 'Verificar si datos recibidos en 1500ms',
	RC_WARNING_CONFLICT:
		'⚠ El transmisor y el receptor no pueden estar activados al mismo tiempo. Desactive uno (clic derecho → Desactivar bloque)',

	// Bloques Lectura de datos RC
	RC_GET_JOYSTICK_PREFIX: 'RC joystick',
	RC_GET_JOYSTICK_TOOLTIP: 'Leer valor joystick remoto (0-4095), 2048 es centro',
	RC_GET_JOYSTICK_MAPPED_PREFIX: 'RC joystick',
	RC_GET_JOYSTICK_MAPPED_MIN: 'mapear a',
	RC_GET_JOYSTICK_MAPPED_MAX: '~',
	RC_GET_JOYSTICK_MAPPED_TOOLTIP: 'Leer joystick remoto y mapear al rango especificado',
	RC_IS_BUTTON_PRESSED_PREFIX: 'RC botón',
	RC_IS_BUTTON_PRESSED_SUFFIX: '¿presionado?',
	RC_IS_BUTTON_PRESSED_TOOLTIP: 'Verificar si botón remoto está presionado',
	RC_GET_BUTTON_PREFIX: 'RC botón',
	RC_GET_BUTTON_SUFFIX: 'estado',
	RC_GET_BUTTON_TOOLTIP: 'Leer estado botón remoto (0=presionado, 1=suelto)',

	// === Serial Monitor ===
	MONITOR_BUTTON_TITLE: 'Abrir monitor',
	MONITOR_BUTTON_STOP_TITLE: 'Detener monitor',
	MONITOR_BUTTON_DISABLED_TITLE: 'Monitor (solo para CyberBrick)',
	MONITOR_STARTING: 'Iniciando monitor...',
	MONITOR_CONNECTED: 'Conectado a {0}',
	MONITOR_STOPPED: 'Monitor detenido',
	MONITOR_DEVICE_NOT_FOUND: 'Dispositivo CyberBrick no encontrado',
	MONITOR_DEVICE_DISCONNECTED: 'Dispositivo CyberBrick desconectado',
	MONITOR_CONNECTION_FAILED: 'Error al conectar con el dispositivo',
	MONITOR_CLOSED_FOR_UPLOAD: 'Monitor pausado para subida',

	// Orphan Block Warnings
	ORPHAN_BLOCK_WARNING_ARDUINO: 'Este bloque debe colocarse dentro de setup(), loop() o una función para generar código.',
	ORPHAN_BLOCK_WARNING_MICROPYTHON: 'Este bloque debe colocarse dentro de main() o una función para generar código.',
	// === AI Shadow Suggestions ===
	AI_SUGGESTION_TAB_HINT: 'Tab para aceptar · Esc para ignorar',
	AI_SUGGESTION_TAB_HINT_MULTI: 'Tab aceptar · Alt+]/[ cambiar · Esc ignorar',
	AI_QUOTA_EXHAUSTED: 'Cuota de Copilot agotada. Se restablece el próximo mes o actualice su plan.',

	// === Sample Browser ===
	SAMPLE_BROWSER_BUTTON_TITLE: 'Cargar ejemplo',
	SAMPLE_BROWSER_TITLE: 'Ejemplos de CyberBrick',
	SAMPLE_BROWSER_LOADING: 'Cargando ejemplos...',
	SAMPLE_BROWSER_OFFLINE_NOTICE: 'Usando la versión incorporada (sin conexión)',
	SAMPLE_BROWSER_LOAD_BUTTON: 'Cargar',
	SAMPLE_BROWSER_EMPTY: 'No hay ejemplos disponibles',
	SAMPLE_BROWSER_CONFIRM_LOAD: '¿Esto reemplazará el espacio de trabajo actual. Continuar?',
	SAMPLE_BROWSER_CONFIRM_YES: 'Cargar ejemplo',
	SAMPLE_BROWSER_CONFIRM_NO: 'Cancelar',
	SAMPLE_BROWSER_ERROR_INVALID: 'Formato de ejemplo inválido, no se puede cargar',

	// === TXT Controller ===
	UPLOAD_BUTTON_TITLE_TXT: 'Subir al TXT Controller',
	TXT_CONNECTION_PANEL_TITLE: 'Configuración de conexión TXT Controller',
	TXT_HOST_LABEL: 'IP del Host',
	TXT_USERNAME_LABEL: 'Usuario',
	TXT_PASSWORD_LABEL: 'Contraseña',
	TXT_REMOTE_PATH_LABEL: 'Ruta remota',
	TXT_SAVE_CONFIG_BTN: 'Guardar configuración',
	TXT_TEST_CONNECTION_BTN: 'Probar conexión',
	TXT_PASSWORD_DEFAULT_PLACEHOLDER: 'Predeterminado: ftc (déjalo vacío para usar el valor predeterminado)',
	TXT_TESTING: 'Conectando...',
	TXT_SSH_SETUP_DONE: '✓ Configuración SSH completada. Las conexiones futuras no requerirán confirmación en la pantalla del TXT.',
	TXT_SSH_CONFIRM_HINT: 'Consejo: Si la pantalla del TXT muestra una confirmación, presiona OK en el dispositivo para permitir el acceso SSH.',
	TXT_CONFIG_SAVED: 'Configuración guardada',
	TXT_UPLOAD_STAGE_CONNECTING: 'Conectando al TXT Controller...',
	TXT_UPLOAD_STAGE_UPLOADING: 'Subiendo programa...',
	TXT_UPLOAD_STAGE_EXECUTING: 'Ejecutando programa...',
	TXT_UPLOAD_STAGE_COMPLETED: 'Completado',
	TXT_UPLOAD_STAGE_FAILED: 'Fallo al subir',
	TXT_RUN_SUCCESS: 'Programa finalizado',
	TXT_RUN_FAILED: 'Error al ejecutar el programa: {0}',
	TXT_STOP_SUCCESS: 'Programa detenido',
	TXT_PREVIEW_READONLY_BADGE: 'Preview only',
	TXT_PREVIEW_READONLY_HINT: 'Preview only. You can scroll this canvas and resize the preview panels, but buttons cannot be edited or pressed.',
	TXT_PREVIEW_EMPTY_STATE: 'No TXT virtual buttons are stored in this backup.',
	TXT_PREVIEW_SPLITTER_LABEL: 'Resize Blockly preview and TXT virtual controls preview',
	TXT_PREVIEW_WARNING_LIST_LABEL: 'TXT preview warnings',
	TXT_PREVIEW_WARNING_LEGACY_MISSING_DOCUMENT: 'This older backup has no TXT virtual controls data yet.',
	TXT_PREVIEW_WARNING_EMPTY_CONTROLS: 'This backup has no TXT virtual buttons.',
	TXT_PREVIEW_WARNING_INVALID_CONTROL_SHAPE: 'Some virtual button data was incomplete; recoverable values are shown with safe defaults.',
	TXT_PREVIEW_WARNING_MISSING_CONTROL_REFERENCE: 'A Blockly block references a virtual button that is missing from this backup.',
	TXT_PREVIEW_WARNING_GENERIC: 'TXT preview data was partially recovered.',
	TXT_PREVIEW_BUTTON_READONLY_TITLE: 'Preview only: this virtual button cannot be edited or pressed here.',
	TXT_VIRTUAL_CONTROLS_TOGGLE_TITLE: 'Toggle TXT virtual controls',
	TXT_VIRTUAL_CONTROLS_TITLE: 'TXT Virtual Controls',
	TXT_VIRTUAL_CONTROLS_MODE_RUNNING: 'Running',
	TXT_VIRTUAL_CONTROLS_MODE_EDITING: 'Editing',
	TXT_VIRTUAL_CONTROLS_ADD_BUTTON: 'Add Button',
	TXT_VIRTUAL_CONTROLS_EMPTY_STATE: 'Create a virtual button here, then read it from Blockly blocks.',
	TXT_VIRTUAL_CONTROLS_RUNNING_HINT: 'Buttons are locked in place while the TXT program is running.',
	TXT_VIRTUAL_CONTROLS_EDITING_HINT: 'Drag buttons to reposition them. Button size follows the display name.',
	TXT_VIRTUAL_CONTROLS_SELECT_PROMPT: 'Select a virtual button to edit its name and colors.',
	TXT_VIRTUAL_CONTROLS_NAME_LABEL: 'Display name',
	TXT_VIRTUAL_CONTROLS_IDENTIFIER_LABEL: 'Identifier',
	TXT_VIRTUAL_CONTROLS_IDENTIFIER_HINT: 'Generated from the display name for MicroPython helper code.',
	TXT_VIRTUAL_CONTROLS_BACKGROUND_LABEL: 'Button color',
	TXT_VIRTUAL_CONTROLS_TEXT_LABEL: 'Text color',
	TXT_VIRTUAL_CONTROLS_DELETE_BUTTON: 'Delete Button',
	TXT_VIRTUAL_CONTROLS_DEFAULT_BUTTON_NAME: 'Button {0}',
	TXT_VIRTUAL_CONTROLS_UNNAMED: 'Button',
	TXT_VIRTUAL_CONTROLS_DELETE_CONFIRM: 'Delete virtual button "{0}"?',
	TXT_VIRTUAL_CONTROLS_DELETE_CONFIRM_REFERENCED: 'Delete "{0}"? {1} Blockly block references will become invalid until you choose another button.',
	TXT_VIRTUAL_CONTROLS_MISSING_OPTION: '{0} (missing)',
	TXT_VIRTUAL_CONTROLS_EMPTY_OPTION: 'Create a button first',
	TXT_VIRTUAL_CONTROLS_UNSELECTED_WARNING: 'Select a virtual button first.',
	TXT_VIRTUAL_CONTROLS_INVALID_WARNING: 'Virtual button "{0}" no longer exists. Choose another button before running.',
	TXT_VIRTUAL_CONTROLS_BLOCKED_TOAST: 'Cannot run until every virtual button block points to an existing button.',
	TXT_VIRTUAL_CONTROLS_RUNTIME_ERROR: 'Virtual controls stopped: {0}',
	TXT_VIRTUAL_CONTROLS_BUTTON_ARIA_LABEL: '{0}. Identifier {1}. State: {2}.',
	TXT_VIRTUAL_CONTROLS_STATE_SELECTED: 'selected',
	TXT_VIRTUAL_CONTROLS_STATE_RUNNING: 'running',
	TXT_VIRTUAL_CONTROLS_STATE_EDITABLE: 'editable',
	TXT_VIRTUAL_CONTROLS_STATE_PRESSED: 'pressed',
	TXT_PREVIEW_BUTTON_ARIA_LABEL: '{0}. Identifier {1}. Preview only, cannot be edited or pressed.',
	TXT_VIRTUAL_BUTTON_STATE_PREFIX: 'read virtual button',
	TXT_VIRTUAL_BUTTON_STATE_TOOLTIP: 'Returns 1 while the selected virtual button is pressed, otherwise 0.',
	// Test Panel UI
	TXT_TEST_PANEL_TITLE: 'Panel de prueba TXT I/O',
	TXT_TEST_PANEL_BUTTON_TITLE: 'Panel de prueba TXT I/O',
	TXT_TEST_MOTORS_TITLE: 'Motores',
	TXT_TEST_OUTPUTS_TITLE: 'Salidas',
	TXT_TEST_INPUTS_TITLE: 'Entradas',
	TXT_TEST_STOP_ALL: 'PARAR TODO',
	TXT_TEST_MOTOR_FWD: '▶ Adelante',
	TXT_TEST_MOTOR_BWD: '◀ Atrás',
	TXT_TEST_STATUS_CONNECTED: 'Conectado',
	TXT_TEST_STATUS_DISCONNECTED: 'Desconectado',
	TXT_TEST_STATUS_PAUSED: 'En pausa',
	TXT_TEST_STATUS_CONNECTING: 'Conectando…',
	TXT_SETUP: 'Inicialización TXT',
	TXT_SETUP_TOOLTIP: 'Inicializa el TXT Controller una vez y prepara recursos de hardware compartidos para todos los procesos TXT.',
	TXT_PROCESS_PREFIX: 'Proceso TXT:',
	TXT_PROCESS_TOOLTIP: 'Define un proceso TXT. El nombre es opcional y solo se usa para mostrar o diagnosticar.',
	TXT_INIT: 'Inicializar TXT Controller',
	TXT_INIT_TOOLTIP: 'Conectar al fischertechnik TXT Controller',
	TXT_MOTOR_SPEED: 'Motor',
	TXT_MOTOR_SPEED_SET: 'ajustar velocidad',
	TXT_MOTOR_SPEED_TOOLTIP: 'Ajustar velocidad del motor (-512 a 512)',
	TXT_M_OUTPUT_PREFIX: 'Salida',
	TXT_COMPONENT_MOTOR: 'Motor',
	TXT_COMPONENT_LAMP: 'Lámpara',
	TXT_LAMP_BRIGHTNESS: 'brillo',
	TXT_M_OUTPUT_STOP: 'Detener salida',
	TXT_M_OUTPUT_TOOLTIP: 'Selecciona un puerto M y un componente, luego ajusta la velocidad del motor o el brillo de la lámpara (0 a 512)',
	TXT_M_OUTPUT_STOP_TOOLTIP: 'Detener la salida del puerto M seleccionado',
	TXT_M_OUTPUT_COMPONENT_CONFLICT_WARNING: 'El puerto M {0} se usa como varios componentes: {1}. Usa un solo tipo de componente por puerto M.',
	TXT_M_OUTPUT_SHARED_PIN_CONFLICT_WARNING: 'El puerto M {0} comparte pines de hardware con {1}. Usa el puerto M o sus salidas O asociadas.',
	TXT_M_OUTPUT_UPLOAD_BLOCKED: 'Se detectó un conflicto de salida TXT M. Corrige los bloques de salida M/O resaltados antes de ejecutar.',
	TXT_M_OUTPUT_CODE_OUTPUT_BLOCKED: 'Se detectó un conflicto de salida TXT M. Corrige los bloques de salida M/O resaltados antes de generar código.',
	TXT_MOTOR_STOP: 'Detener motor',
	TXT_MOTOR_STOP_TOOLTIP: 'Detener el motor especificado',
	TXT_OUTPUT: 'Salida',
	TXT_OUTPUT_SET: 'ajustar',
	TXT_OUTPUT_TOOLTIP: 'Activar o desactivar la salida',
	TXT_INPUT_READ: 'Leer entrada',
	TXT_INPUT_READ_TOOLTIP: 'Leer valor de entrada digital',
	TXT_INPUT_SENSOR_PREFIX: 'Leer',
	TXT_SENSOR_BUTTON: 'Botón',
	TXT_SENSOR_GATE: 'Barrera óptica',
	TXT_SENSOR_ULTRASONIC: 'Ultrasonido',
	TXT_INPUT_SENSOR_TOOLTIP: 'Seleccionar tipo de sensor y leer valor (botón/barrera óptica devuelve 0 o 1, ultrasonido devuelve distancia en cm)',
	TXT_WAIT: 'Esperar',
	TXT_WAIT_UNIT: 'ms',
	TXT_WAIT_TOOLTIP: 'Esperar el número de milisegundos especificado',
	TXT_STOP_ALL: 'Detener todos los motores y salidas',
	TXT_STOP_ALL_TOOLTIP: 'Detener todos los motores y desactivar todas las salidas',
	TXT_DIRECTION_FORWARD: 'adelante',
	TXT_DIRECTION_BACKWARD: 'atrás',
	TXT_STATE_ON: 'ENCENDIDO',
	TXT_STATE_OFF: 'APAGADO',
		TXT_MAIN: 'Programa Principal TXT',
	TXT_MAIN_TOOLTIP: 'El punto de entrada principal del TXT Controller. Todos los bloques deben colocarse aquí.',
	TXT_ORPHAN_WARNING: 'Este bloque debe colocarse dentro del bloque "Programa principal TXT"',
	TXT_ORPHAN_WARNING_MULTI: 'Este bloque debe colocarse dentro de un bloque "Inicialización TXT" o "Proceso TXT"',
	TXT_SETUP_DUPLICATE_WARNING: 'Se detectaron varios bloques "Inicialización TXT". Elimina los bloques adicionales.',
	TXT_SETUP_REQUIRED_WARNING: 'El espacio de trabajo TXT requiere un bloque "Inicialización TXT".',
	TXT_PROCESS_REQUIRED_WARNING: 'El espacio de trabajo TXT requiere al menos un bloque "Proceso TXT".',
	CATEGORY_TXT: 'TXT Controller',

        PENV_PROVIDER_INSTALL_FAILED: 'La instalación automática falló. Por favor instale "PlatformIO IDE" (VS Code Marketplace) o "pioarduino" (Open VSX para VSCodium) desde el panel de extensiones.',
        PENV_PROVIDER_PENDING: 'Entorno PlatformIO inicializándose, por favor espere...',

        PLATFORMIO_UNPINNED_PLATFORM_WARNING: 'Este proyecto existente usa una plataforma PlatformIO sin versión fija ({0}). No se modificó. Fija la versión de la plataforma antes de una publicación reproducible.',
        PENV_PROVIDER_RELOAD_REQUIRED: 'La extensión de entorno PlatformIO se instaló correctamente. Recargue VS Code ahora para completar la configuración.',
        PENV_PROVIDER_RELOAD_BUTTON: 'Recargar ahora',

	FEEDBACK_PANEL_TITLE: "Enviar comentarios",
	FEEDBACK_INTRO: "Cuéntanos qué ocurrió o cómo podríamos mejorar Singular Blockly. No necesitas una cuenta de GitHub.",
	FEEDBACK_KIND_LABEL: "Tipo de comentario",
	FEEDBACK_KIND_BUG: "Informar un problema",
	FEEDBACK_KIND_FEATURE: "Sugerir una función",
	FEEDBACK_KIND_QUESTION: "Pregunta de uso",
	FEEDBACK_KIND_OTHER: "Otro comentario",
	FEEDBACK_TITLE_LABEL: "Resumen breve",
	FEEDBACK_DESCRIPTION_LABEL: "¿Qué te gustaría contarnos?",
	FEEDBACK_STEPS_LABEL: "Pasos para reproducir el problema (opcional)",
	FEEDBACK_EXPECTED_LABEL: "¿Qué resultado esperabas? (opcional)",
	FEEDBACK_BASIC_DIAGNOSTICS_LABEL: "Incluir información básica del entorno",
	FEEDBACK_BASIC_DIAGNOSTICS_HELP: "Incluye versiones de la aplicación, familia del sistema operativo, arquitectura, idioma y tipos de host y espacio de trabajo. Nunca incluye código fuente, rutas, identificadores de dispositivos, datos de red ni registros sin procesar.",
	FEEDBACK_DIAGNOSTICS_OFF_WARNING: "La información básica del entorno está desactivada. Puedes enviar tus comentarios de todos modos, pero el diagnóstico puede ser menos eficaz.",
	FEEDBACK_RECENT_EVENTS_LABEL: "Incluir eventos estructurados recientes",
	FEEDBACK_RECENT_EVENTS_HELP: "Está desactivado de forma predeterminada. Solo incluye una cantidad limitada de horas, etapas, códigos de evento estables y resultados.",
	FEEDBACK_REVIEW_ACTION: "Revisar lo que se enviará",
	FEEDBACK_REVIEW_TITLE: "Revisar antes de enviar",
	FEEDBACK_REVIEW_HELP: "Estos son todos los datos que se enviarán. No se transmitirá nada hasta que confirmes abajo.",
	FEEDBACK_BACK_ACTION: "Volver",
	FEEDBACK_CONFIRM_ACTION: "Confirmar y enviar",
	FEEDBACK_SENDING: "Enviando comentarios…",
	FEEDBACK_SUCCESS: "Gracias. Recibimos tus comentarios.",
	FEEDBACK_REFERENCE_LABEL: "Número del comentario",
	FEEDBACK_COPY_RECOVERY_ACTION: "Copiar enlace de acceso de respaldo",
	FEEDBACK_RECOVERY_COPIED: "Enlace de acceso de respaldo copiado. Mantenlo en privado.",
	FEEDBACK_ERROR_GENERIC: "No se pudieron enviar los comentarios. Revisa el formulario e inténtalo de nuevo.",
	FEEDBACK_ERROR_TIMEOUT: "La solicitud agotó el tiempo de espera. El formulario sigue aquí; intenta enviarlo de nuevo.",
	FEEDBACK_ERROR_RATE_LIMITED: "Se hicieron demasiadas solicitudes. El formulario sigue aquí; espera un momento e inténtalo de nuevo.",
	FEEDBACK_ERROR_SERVICE_UNAVAILABLE: "El servicio de comentarios no está disponible temporalmente. El formulario sigue aquí; inténtalo más tarde.",
	FEEDBACK_ERROR_ATTACHMENT: "Se rechazó la captura de pantalla. Quítala o elige una imagen PNG o JPEG recién capturada y vuelve a revisarla.",
	FEEDBACK_ERROR_VALIDATION: "Algunos campos no son válidos. Revisa la información marcada e inténtalo de nuevo.",
	FEEDBACK_ERROR_SENSITIVE_CONTENT: "Se detectó información confidencial. Elimina credenciales, rutas privadas, direcciones IP, código fuente o salidas del terminal y vuelve a revisar.",
	FEEDBACK_SCREENSHOT_LEGEND: "Captura de pantalla opcional",
	FEEDBACK_SCREENSHOT_CHOOSE: "Elegir una captura de pantalla",
	FEEDBACK_SCREENSHOT_HELP: "La imagen se vuelve a codificar en este dispositivo, se limita a 1920 píxeles y 3 MiB, y solo se envía después de tu confirmación.",
	FEEDBACK_SCREENSHOT_PRIVACY: "Antes de enviarla, comprueba que la imagen no contenga nombres, correos electrónicos, rutas, contenido del programa ni otros datos privados.",
	FEEDBACK_SCREENSHOT_REMOVE: "Quitar captura de pantalla",
	FEEDBACK_SCREENSHOT_ERROR: "No se pudo preparar esta imagen. Elige una imagen PNG o JPEG dentro de los límites.",
	FEEDBACK_DATA_LEGEND: "Información enviada con tus comentarios",
	FEEDBACK_SCREENSHOT_PREVIEW_ALT: "Vista previa de la captura elegida",
	FEEDBACK_REVIEW_SCREENSHOT_ALT: "Captura de pantalla que se enviará",
	FEEDBACK_OPEN_FAILED: "No se pudo abrir el formulario de comentarios. Inténtalo de nuevo.",
	FEEDBACK_PERSONAL_DATA_WARNING: "No incluyas nombres, correos, datos de estudiantes, credenciales, código fuente, rutas privadas ni registros completos.",
	FEEDBACK_SERVICE_DISCLOSURE: "Al enviar, el comentario se guarda en Cloudflare D1/R2 y en un espacio privado de GitHub hasta que lo elimines. Las copias del proveedor pueden persistir brevemente. Solo se crea un resumen público anonimizado con aprobación del responsable del proyecto.",
	FEEDBACK_PROVIDE_ACTION: "Enviar comentarios",
	FEEDBACK_MY_FEEDBACK_ACTION: "Mis comentarios",
	FEEDBACK_MY_FEEDBACK_TITLE: "Mis comentarios",
	FEEDBACK_MY_FEEDBACK_HELP: "Aquí solo aparece lo enviado con esta credencial privada de recuperación.",
	FEEDBACK_DETAIL_DESCRIPTION_LABEL: "Descripción",
	FEEDBACK_DETAIL_STEPS_LABEL: "Pasos para reproducir",
	FEEDBACK_DETAIL_EXPECTED_LABEL: "Resultado esperado",
	FEEDBACK_DETAIL_DIAGNOSTICS_LABEL: "Información del entorno",
	FEEDBACK_DETAIL_ATTACHMENT_INCLUDED: "Captura de pantalla adjunta.",
	FEEDBACK_DETAIL_ATTACHMENT_NOT_INCLUDED: "No hay ninguna captura de pantalla adjunta.",
	FEEDBACK_EMPTY: "Esta instalación aún no ha enviado comentarios.",
	FEEDBACK_LOAD_MORE: "Cargar más",
	FEEDBACK_ADD_MESSAGE_ACTION: "Añadir información",
	FEEDBACK_MESSAGE_LABEL: "Información adicional",
	FEEDBACK_DELETE_ONE_ACTION: "Eliminar este comentario",
	FEEDBACK_DELETE_ALL_ACTION: "Eliminar todos los comentarios y el acceso de recuperación",
	FEEDBACK_DELETE_ONE_HELP: "Escribe DELETE para confirmar.",
	FEEDBACK_DELETE_ALL_HELP: "Escribe DELETE ALL para confirmar. También se invalidará el enlace de recuperación.",
	FEEDBACK_DELETION_BACKUP_NOTICE: "El contenido principal se elimina de inmediato; las copias cifradas pueden permanecer brevemente en respaldos y puede conservarse un registro público de desarrollo anonimizado aprobado por el responsable.",
	FEEDBACK_OPERATION_SUCCESS: "El cambio se guardó.",
	FEEDBACK_PRIVACY_ACTION: "Privacidad",
	FEEDBACK_SUPPORT_ACTION: "Soporte",
	FEEDBACK_TERMS_ACTION: "Términos",
	FEEDBACK_NAVIGATION_LABEL: "Secciones de comentarios",
	FEEDBACK_STATUS_LABEL: "Estado",
	FEEDBACK_CREATED_LABEL: "Creado",
	FEEDBACK_AUTHOR_REPORTER: "Tú",
	FEEDBACK_AUTHOR_MAINTAINER: "Responsable",
	FEEDBACK_STATUS_RECEIVED: "Recibido",
	FEEDBACK_STATUS_TRIAGING: "En revisión",
	FEEDBACK_STATUS_NEEDS_INFO: "Se necesita información",
	FEEDBACK_STATUS_PLANNED: "Planificado",
	FEEDBACK_STATUS_IN_PROGRESS: "En curso",
	FEEDBACK_STATUS_RESOLVED: "Resuelto",
	FEEDBACK_STATUS_CLOSED: "Cerrado",
});
