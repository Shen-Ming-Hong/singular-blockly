...
- **i18n key 組**：共 5 個新 key（`CYBERBRICK_LED_DIGITAL_PREFIX`、`CYBERBRICK_LED_DIGITAL_ON`、`CYBERBRICK_LED_DIGITAL_OFF`、`CYBERBRICK_LED_DIGITAL_TOOLTIP`、`X11_LED_DIGITAL_TOOLTIP`）；`x11_led_digital` 的介面標籤重用現有 key（`X11_LED_SET_COLOR_PREFIX`、`X11_LED_SET_COLOR_INDEX`、`X11_LED_SET_COLOR_INDEX_SUFFIX`），不新增對應標籤 key
