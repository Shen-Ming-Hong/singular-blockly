...
- **FR-001**: 系統在 Arduino 上傳時 MUST 先執行編譯（確認程式碼語法正確），編譯成功後再嘗試上傳。若上傳階段因無裝置而失敗，系統 MUST 從 PlatformIO 的 stderr 分類錯誤並提示「未偵測到硬體」。「僅編譯」模式 MUST 跳過上傳階段，直接回報編譯結果
...
- **FR-005**: 系統 MUST 在編譯與上傳過程中顯示對應的進度提示（如「編譯中...」、「上傳中...」）
- **FR-006**: 上傳過程中的連接埠錯誤（佔用、斷線、逾時）MUST 各自對應明確的錯誤訊息
- **FR-007**: CyberBrick (MicroPython) 的上傳流程和錯誤訊息 MUST NOT 受此功能影響
- **FR-008**: 當上傳過程中 PlatformIO 回報無法辨識的錯誤時，系統 MUST fallback 為通用的「上傳失敗」錯誤訊息，並附帶原始技術細節
...
- PlatformIO CLI 編譯時會自動檢查語法錯誤，上傳時會自動偵測裝置，失敗的 stderr 輸出包含足夠資訊來分類錯誤原因
...
- **SC-001**: 使用者在未連接硬體時點擊上傳，系統會先編譯（確認程式碼語法），接著在上傳階段顯示「未偵測到硬體」的明確提示
