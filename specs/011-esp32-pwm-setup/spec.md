...
教育者或學生需要使用 ESP32 控制支援高頻 PWM 的馬達驅動晶片（如 TB6612、DRV8833 等），這些晶片需要至少 20-75KHz 的 PWM 頻率才能正常運作並避免馬達產生高頻噪音。使用者希望透過視覺化積木介面設定 PWM 參數，而不需要撰寫複雜的 C++ 程式碼。
...
-   **超出頻率上限**：當使用者輸入頻率 > 80000Hz 時，系統將頻率限制在 80000Hz 並生成警告註解
-   **超出頻率下限**：當使用者輸入頻率 < 1Hz 時，系統將頻率限制在 1Hz 並生成警告註解
-   **非 ESP32 開發板選擇時**：PWM 設定積木在 toolbox 中不顯示或呈現禁用狀態，避免使用者誤用
...
-   **FR-008**: 系統必須僅在使用者選擇 ESP32 開發板時，在積木工具箱中顯示 ESP32 PWM 設定積木
...
-   **SC-007**: 90% 的使用者能透過積木的 tooltip 說明理解頻率與解析度的限制關係，無需額外技術支援
-   **SC-008**: 生成的程式碼註解清晰標示 PWM 配置和驗證結果，開發者能在 30 秒內理解當前設定
-   **SC-009**: 程式碼生成作業在 500ms 內完成，保持與現有積木一致的即時回饋體驗
...
-   除繁體中文和英文外的語言翻譯不在初始實作範圍（Phase 2 補充）
