...
> **本 PR 實際交付**：OTA Agent 自動升級（US2）+ 舊版優雅降級（US3）+ Monitor 路由優化 + agent 1.5.9。
...
2. **Monitor 按鈕路由優化**：Monitor 按鈕現在有 USB 連接時優先走 USB serial monitor，提高裝置連線穩定性。OTA WiFi Monitor 路由已移除。
