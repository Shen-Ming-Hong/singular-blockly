...
學生在 VS Code 上首次開啟任一 Singular Blockly 工作區，尚未安裝 PlatformIO IDE。擴充功能沿用舊版的 PlatformIO 前置需求，改以執行期一鍵安裝提供環境。
...
1. **給定** 任一板子工作區未安裝任何 penv provider，**當** 學生開啟積木編輯器，**則** VS Code 顯示安裝確認並開始安裝 provider。
...
使用 TXT Controller 或 CyberBrick OTA 工作區的使用者，與舊版 `extensionDependencies` 行為一致，仍會要求安裝 PlatformIO provider；差異僅在安裝時機改成積木編輯器開啟後由 VS Code 確認。
