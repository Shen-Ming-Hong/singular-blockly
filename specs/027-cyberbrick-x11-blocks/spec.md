...
### ~~User Story 5 - 使用平滑移動功能讓伺服馬達緩慢轉動~~ (已移除)
...
-   ~~**FR-004**: 系統必須提供伺服馬達(180°)平滑移動積木~~ (已移除 - CyberBrick API 有 bug)
...
-   ~~**FR-010**: 系統必須提供關閉 LED 燈條積木~~ (已移除 - 使用者可設定 RGB=(0,0,0) 達成相同效果)
-   ~~**FR-011**: 當使用伺服馬達平滑移動積木時，系統必須自動注入 timing_proc()~~ (已移除)
...
-   **FR-016**: LED 燈條積木必須將 D1/D2 埠位轉換為對應的 GPIO 腳位（D1→GPIO 21, D2→GPIO 20，依據 bbl.leds 源碼定義）
...
-   **SC-003**: 所有 6 種積木的 Tooltip 皆能正確顯示，幫助使用者理解適用硬體和使用方式
