...
學生在工作區可找到完整的 TXT 積木類別，包含：TXT 初始化、設定馬達速度（M1-M4）、停止馬達、設定輸出狀態（O1-O8）、讀取輸入（I1-I8）、等待指定毫秒、全部停止；同時也能在「迴圈」類別快速找到兒童友善的無限循環積木，用簡單易懂的名稱建立持續運作的程式。所有積木在 15 種語言介面下均有正確翻譯。
...
4. **Given** 使用者想讓程式持續重複執行，**When** 開啟「迴圈」類別，**Then** 能找到一個以兒童友善文案呈現的無限循環積木，無需自行組合布林常數即可開始重複執行內部指令
...
- **FR-025**: 系統 MUST 在「迴圈」工具箱類別提供一個兒童友善的無限循環積木，讓使用者不需自行組合布林常數即可持續重複執行內部指令
