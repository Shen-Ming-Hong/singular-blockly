...
import { CYBERBRICK_OTA_AGENT_TARGET_VERSION } from '../../types/cyberbrickUpload';
...
suite('MicropythonUploader CyberBrick helper commands', () => {
	...
	test('builds OTA agent source without patch artifacts and with background startup', () => {
		...
		assert.ok(source.includes('def _write_uploaded_file(client, leftover, content_length, path):'), 'OTA agent should share a streaming file writer for OTA uploads');
