...
suite('CyberBrickOtaUploader', () => {
	...
	test('retries with reset when the direct upload window times out', async () => {
		const device = await seedReadyDevice();
		fetchStub.onFirstCall().resolves(jsonResponse(200, {
			deviceId: device.deviceId,
			agentVersion: CYBERBRICK_OTA_AGENT_TARGET_VERSION,
		...
		assert.strictEqual(result.status, 'succeeded');
		assert.strictEqual(fetchStub.callCount, 5, 'upload should retry once with a reset-opened window after the direct attempt times out');
		assert.strictEqual(fetchStub.getCall(2).args[0], 'http://192.168.1.50:8266/api/v1/reset');
