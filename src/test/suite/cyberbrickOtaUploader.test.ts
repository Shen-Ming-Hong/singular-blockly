...
suite('CyberBrick OTA Uploader - upload() pipeline', () => {
	...
	test('direct upload timeout triggers reset fallback and retries the main upload', async () => {
			const device = makeDevice({ agentVersion: CYBERBRICK_OTA_AGENT_TARGET_VERSION });
			const settingsSvc = makeSettingsService(sandbox, device);
			const capturedUrls: string[] = [];
			let uploadAttempt = 0;
			...
			assert.strictEqual(run.status, 'succeeded');
			assert.ok(capturedUrls.some(u => u.includes('/reset')), 'reset fallback should be attempted after the direct upload window is missed');
